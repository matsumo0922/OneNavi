...
<p align="center">
Google マップなどから共有するだけで、すぐに経路案内をはじめられる。<br>
OneNavi は <b>経路案内に特化</b> した Android 向けカーナビアプリです。
</p>
...
`drive-supporter-api`** に委譲しています。このライブラリは非公開のため、現状では
**本リポジトリ単体ではビルドできません**。あらかじめご了承ください。

現在は、`drive-supporter-api` の **interface 互換を保ったまま Google Maps SDK / Google
...
- **案内情報は `drive-supporter-api`** から受け取り、日本のカーナビに必要なレーン・方面
  看板・規制・渋滞・信号案内を扱う
...
必要な情報を、見やすく・正しいタイミングで届けることを目指しています。OSS として公開し、
収益化は行いません。
...
- [kotlinx.collections.immutable](https://github.com/Kotlin/kotlinx.collections.immutable)（Compose 安定性）
...
- `drive-supporter-api` への依存を外し、Google Maps SDK / Google Routes API のみで
...
- **案内情報**は `drive-supporter-api` から受け取り、`ExtNav*` レイヤーでアプリの
...
前述のとおり、経路案内のコアロジックは private な `drive-supporter-api` に依存している

...
- OneNavi が UI 仕様として許可する tail direction

現状 OneNavi の `CalloutTailDirection` は 4 隅を持つので、内部表現には対応できる。
Google Maps アプリの見た目へ寄せるなら、route preview では `BottomRight`, `BottomLeft` を優先し、
上側 anchor は fallback または検証対象にする。
...
OneNavi の現状 `CalloutShape` は、全方向に tail 余白を固定で確保し、tail 方向が変わっても bounds が変わらない。
これは SubcomposeLayout の計測安定性には有利だが、SDK の geometry とは完全一致しない可能性がある。
...
- bounds が anchor ごとに変わるか、OneNavi のように固定 bounds でよいか

## OneNavi 現状実装との差分

対象: `core/ui/src/androidMain/kotlin/me/matsumo/onenavi/core/ui/callout/`

| 領域 | 現状 | SDK 推定仕様との差 |
|---|---|---|
| 候補選択 | `candidates + primaryPoint` を順に見る | visible polyline を最大 10 点にサンプリングし、中央寄りから見る |
| anchor 選択 | `CalloutTailDirection.entries` の順 | supported anchors を前回 anchor 起点で rotate |
| 評価関数 | 画面内、既存 Callout overlap の first-fit | weighted scorer。hard penalty / soft penalty / reward がある |
| 前回位置維持 | なし | 前回 position + anchor を先に試す。score `0.8` 超で維持 |
| route line 回避 | なし | polyline 被覆 scorer が強い |
| exclusion rect | なし | viewport / exclusion を見る |
| priority | 入力順のみ | use case / priority / identity でソート |
| 非表示条件 | 基本的に必ず配置 | score が 0 以下なら非表示 |
| fixed Callout | `AnchorFirst` は画面内優先のみ | 固定地点で anchor を scoring する |
| temporal stability | なし | 前回位置、前回 anchor、zoom 変化で制御 |

## OneNavi 側の完全一致実装方針

### 1. 配置 engine を first-fit から scorer 方式に変える

`CalloutPlacement.compute()` は pure function のまま維持し、内部を次の構造に置き換える。

```text
CalloutPlacement.compute(inputs):
  ...
  for callout in sortedCallouts:
    ...
    result = positioner.place(context)
    if result.visible:
...
### 2. Callout input を拡張する

現状の `CalloutAnchor` だけでは情報が足りない。

追加したい情報:

- `useCase`: route duration / navigation maneuver / incident / decoration
- `priority`: 同一 useCase 内の表示順
- `supportedDirections`: `BottomRight`, `BottomLeft`, `TopRight`, `TopLeft`
- `previousPlacement`: 前回 tip, tailDirection, zoom
- `routeScreenPolylines`: polyline scorer 用
- `exclusionRects`: top app bar、bottom controls、FAB、案内パネルなど
- `importantPoints`: maneuver point、waypoint marker、destination marker など

### 3. ルート Callout の候補生成を合わせる

`CalloutAnchor.Flexible.candidates` を caller が作るのではなく、route polyline と viewport から engine が作る。
...
anchors = rotate(supportedDirections, start = previousDirection)
maxAttempts = 40
```
...
| `AnchorStickinessScorer` | 前回 direction と同じか |
| `PositionStickinessScorer` | 前回 tip + direction と同じか |
| `ManeuverDirectionScorer` | maneuver の incoming / outgoing bearing と direction の相性 |
...
- `CalloutPlacement` を scorer 方式に置き換える
- `CalloutPlacement.Result` に `visible` または非表示表現を追加する
- `supportedDirections` と `previousPlacement` を入力できるようにする
- 既存 UI を壊さない adapter を用意する
...
- OneNavi の固定 bounds 方式で許容できるか確認する
...
weighted scoring として扱われている。完全一致を狙うなら、現在の first-fit 実装を scorer 型に置き換えるのが必須。

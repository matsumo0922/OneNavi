...
- ~~**未達**: Gate E(走行維持)~~ → **2026-06-07 に入口特定 + DHU "restrict all" で実証成功**(後述「🎯 Gate E 入口特定」「第2経路 `lxi`」「STEP2 実証成功」)。実車のギア/速度経路での維持は未実証。
...
**🎉 STEP2 実証成功(2026-06-07 / DHU "restrict all")**: E-1 + E-2 を有効化し、**走行制限下でもトーストも teardown も発生せず OneNavi が車画面に維持される**ことを確認。スタックトレース診断(`iij.l`/`y`/`s` に一時 hook)で teardown 経路も確定:

- 閉じる時の teardown は `xl.run`(posted Runnable)→ `jil.m` → `ifz.a` → **`ihu.af()`** → `iij.l()`(finishManager)。`ihu` = 活性 manager の親、`iij.l` = NativeAppCarActivityManager の終了。
- もう1本の trace は `iij.l` → `iij.P`(状態機械)→ `ihu.aw` 経由 = **「別アプリへ置換した結果 ihu が旧 manager を finish」**。つまり teardown は **E-1(`lxl.eJ`)の置換動作の下流**。∴ E-1 を no-op にすると置換が起きず `ihu.af`→`iij.l` も呼ばれない(実測で teardown 消失)。
- ∴ `iij.l`(teardown)を直接 hook する必要はない。**上流の E-1/E-2 を塞げば十分**。

> ⚠️ **運用上の罠(重要)**: LSPosed/Vector の hook は「次に起動する gearhead プロセス」にのみ当たる。モジュールを入れ直す/更新するたびに **gearhead を `force-stop` しないと旧プロセスに hook が乗らず「有効化したのに走行中閉じる」**状態になる(初回の失敗原因はこれだった)。

**残る未確認**: UXR 系(`iet`/`ieu` の "always restricted")は別系統のコンテンツ制限(完全 stop でなく UI ブランク/操作制限)の可能性。今回 DHU "restrict all" では E-1/E-2 で維持できたため当面問題ないが、実車のギア/速度経路で挙動差が出れば `iet` 側を別途 trace する(未着手)。

---

### 🟣 経路B(projection split)実証(2026-06-07)— Strato relay 合成への割り込み

split 同居(Coolwalk)がマスト要件として再浮上したため、`docs/logs/12` で一度棚上げした projection 経路を再検討し、**新 SCVH relay 合成ホスト `ProjectionRootActivity` への割り込みで描画+本物入力が成立する**ことを実機(Android16/DHU)で実証した。詳細 `docs/logs/13`。

- projection は2世代: 旧 `GhostActivity`+`DrawingSpec`(VirtualDisplay/Presentation、実機現行) / 新 `ProjectionRootActivity`(CAR.WM.ProjRootAct、SCVH relay、display name "RelayComposer")。
- 新経路 gate: gearhead `igv`(line 1888-1940)が `acsl.a()`(=`acsm.a()`=Phenotype **`StratoFeature__enabled`**, default false)が true **かつ** `SDK_INT>=36` で `jcl`→`ProjectionRootActivity` を `setLaunchDisplayId` 起動。Maps 側 `binx.l` も `SDK_INT>=36 && bimp.j(54)` で新 `bioo`(SCVH)。実機 Android16 は SDK 条件は満たすが Phenotype 未配信で false → 旧経路だった。**hook 対象は実装本体 `acsm.a()`**(igv は `acsl.a()` を直呼びするため `acsk.c()` hook では効かない)。
- 実証: `acsm.a()` を true 固定(Strato 解錠)→ `ProjectionRootActivity` 起動 → その rootLayout(`FrameLayout` instance field `c`)に赤箱を `addView` → **HU 描画 + `PROBE TOUCH`(本物 MotionEvent)** を確認。native(専用 display 全画面占有)では不可能だった合成フレームへの割り込みが描画も入力も成立。
- 課題: host 単独 Strato 強制では Maps 等 client が新 SCVH 経路に追従できず破綻(「マップ読み込みエラー」、rootLayout に相手 SurfaceView 来ず赤箱のみ)。**真の split 同居には相手 app も新経路(Maps なら `bimp.j(54)`)の有効化が必要**(次段で検証)。
- 実装: `xposed-carunlock/ProjectionSplitProbe.kt`(実験用・確認後削除)。

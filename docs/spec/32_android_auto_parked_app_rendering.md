...
| **E** runtime driving / UXR | car parked-app の走行制限系。**評価**=`iet`("No driving status, always restricted")、**センサ源**=`qno`(`CAR.SENSOR`)。`qhi`("removing active app for the region")は **Activity が stopped になった後段の registry cleanup(結果観測点)**で走行起点ではない(`qpy` 等の proxy Activity lifecycle stopped 通知が起点)。**走行開始時に誰が active app を stop させるかの実呼び出し元は未特定 → 要 trace** | `iet:266`/`qno`/`qhi:594`/`qpy:811` |
...
### 3.4 ゲートC: VDM activity policy(Codex 指摘の中心ゲート)

- gearhead は VirtualDevice を `setAllowedActivities(Set)`(`bwn:1294`)で構築。**OneNavi の CarActivity component を許可集合に入れる**必要。
- 手段: `setAllowedActivities` の集合に OneNavi を加える hook / `addActivityPolicyExemption()`(`ijf:4254`)/ `CradleFeature__allowed_activities_list` 上書き。
- これを通さないと、A/B/D が揃っても **VDM policy で第三者 Activity 起動が弾かれる**(失敗の最有力原因)。
...
### 3.6 ゲートE: runtime driving / UXR(走行中維持の本丸)

- car parked-app の走行制限系: **評価**=`iet`("No driving status, always restricted")、**センサ源**=`qno`=CAR.SENSOR。`qhi`("removing active app for the region")は **Activity stopped 後の registry cleanup(結果観測点)で走行起点ではない**(`qpy` の lifecycle 通知が起点)。
- **走行開始時に誰が active app を stop させるか(Gate E の入口)は未特定** → `iet`/`qno` から「走行検知 → stop 実行」の実呼び出し元を trace してから hook 点を決める(§6-1)。**決め打ち禁止**(Codex 指摘)。
- **`ldw`/`lek`→0(=lzt/スマホ系)では効かない見込み**。∴ **D2 の「速度0グローバル」は E に不十分**。スコープ/手段は trace 後に確定。
...
C VDM activity policy:  ← Codex 指摘の中心ゲート
  setAllowedActivities(Set) に OneNavi component を含める / addActivityPolicyExemption()
  （or CradleFeature__allowed_activities_list 上書き）

D car launch:
  iij（hook 不要。A/B/C が通れば iij が setLaunchDisplayId(car) で起動）

E runtime driving（走行維持。hook 点 未確定 → trace 後に決定）:
  入口未特定。iet(評価)/qno(CAR.SENSOR) から「走行検知 → active app stop」の実呼び出し元を trace してから決める
  ※ qhi は stopped 後の cleanup(結果観測点)なので hook 点に決め打ち禁止（Codex 指摘）
  ※ ldw/lek→0 は lzt(スマホ)系で E には効かない見込み
...
1. **[最優先・STEP2 の前提] ゲートE の入口特定**。走行開始時に **誰が active app を stop/remove するか**の実呼び出し元を trace(`iet`=評価 / `qno`=CAR.SENSOR から辿る)。`qhi:594`(`qpy:811` の lifecycle stopped 通知 → registry cleanup)は**結果観測点で起点ではない**ので hook 点に決め打ちしない(Codex 指摘)。**`ldw`/`lek`→0 は効かない見込み**。最大の破綻点(Codex F3)。
2. **[最優先] ゲートC(VDM activity policy)**。`setAllowedActivities`/`addActivityPolicyExemption`/`CradleFeature__allowed_activities_list` のどれで OneNavi を許可集合に入れるか実機確認。第三者 Activity 起動の山場(Codex F6)。
...
- gearhead smali(全て検証済):
  ...
  - ゲートC: `bwn.smali:1294`(`setAllowedActivities`)、`ijf.smali:4254`(`addActivityPolicyExemption`)
  - ゲートD: `iij.smali:1348`(`setLaunchDisplayId(carDisplayId)`)
  - ゲートE: `iet.smali:266`(評価)、`qno.smali`(CAR.SENSOR)。`qhi.smali:594` は stopped 後 cleanup(`qpy.smali:811` の lifecycle 通知が起点)= 結果観測点。**走行 stop の実呼び出し元は未特定(要 trace)**

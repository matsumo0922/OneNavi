...
> **関連:** `17_callout_redesign.md`, `21_ext_api_guide_proto_and_announcement.md`,
...
> `33_external_api_local_dependency.md`
...
外部API ライブラリ依存方式は、本書で作り直さない。

公開 OSS 側の文書なので、外部API 提供事業者・製品の実名、private repository の URL、
認証情報、アセット由来を特定できる情報は記載しない。必要な呼称は
「外部API ライブラリ」「外部API 提供元」「外部API の参照実装アプリ」に統一する。
...
| 案内中 state 配布 | spec 28、`ExtApiGuidanceTracker.snapshot` → `NewGuidanceManager.state` → `MapViewModel.newGuidanceState` | A/C は route の静的データとして流す。B は別 StateFlow に分ける |
| 外部API ライブラリ依存 | spec 33 | OneNavi 側は通常 AAR を読む。private checkout の変更は別 PR |
...
```text
ExtApiRouteDataSource.searchRoutes()
...
`ExtApiTrafficOverlay` / `RegulationOverlay` 契約を正とし、本書では置き換えない。
...
|---|---|---|---|
| A: 信号機 / 一時停止 / 踏切 | 発話 category と `GuidancePoint.distanceFromStartMetres` | 外部API ライブラリ側で polyline 補間し `pointEvents` として公開 | なし。route 取得時に受ける |
...
したがって、外部API ライブラリ側で `GuidancePoint.distanceFromStartMetres` を
...
### 4.1 外部API ライブラリ側の依存事項

外部API ライブラリに次の domain を追加して公開する。
...
`ExtApiRouteDataSource.searchRoutes()` では `buildCongestionSegments()` と同じ場所で
`buildPointEvents()` を呼び、外部API ライブラリ側の index を OneNavi の `geometry` index に
offset 補正して詰める。
...
`ExtApiGuidanceTracker` は point event を所有しない。現在位置より手前の event を隠すかどうかは、
描画実装側が `GuidanceProgress.currentCumulativeMeters` と `RoutePointEvent.distanceFromStartMeters`
を組み合わせて判断する。

### 4.4 描画実装時の注意

- アイコンは clean-room で自作し、外部API の参照実装アプリや他社アセットをトレースしない。
...
| 層 | 責務 |
|---|---|
| `ExtApiTrafficOverlay` | spec 28 の動的交通 overlay 契約を正本として、案内開始 / 5 分 / 5 km / reroute の trigger を管理する |
| `RegulationOverlay` | 規制・事故の取得、route 近接フィルタ、`RegulationOverlayState` の公開を担当する |
| `NewGuidanceManager` | guidance lifecycle に合わせて `ExtApiTrafficOverlay` / `RegulationOverlay` を start / stop する |
...
`RegulationOverlay` は `ExtApiGuidanceTracker.snapshot` を読んで現在累積距離や走行距離を判断してよい。
...
将来 spec 28 の `ExtApiTrafficOverlay` が最新渋滞 segment を `TrafficOverlayState` として公開した
...
2. **A: 信号機 / 一時停止 / 踏切アイコン**
   - 外部API ライブラリで `RouteGuidance.pointEvents` を公開する。
...
外部API ライブラリ自体は spec 33 の通り、通常は local file Maven repository の AAR として解決する。
...
| B の pathCode 取得 | 外部API ライブラリ側の route DTO 露出が必要 | ライブラリ側 PR |

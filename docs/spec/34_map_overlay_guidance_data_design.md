...
```text
ExtNavTrafficDataSource
  └ TrafficOverlayManager
       └ TrafficOverlayState
            └ MapViewModel.trafficOverlayState
                 └ MapEffect
```

`TrafficOverlayState` の失敗は案内進捗を止めない。取得に失敗した場合は最後に成功した overlay を
保持し、UI は stale であることを必要に応じて表現する。
...
`TrafficOverlayState` は `GuidanceProgress` に混ぜず、別 state として公開する。
...
| 層 | 責務 |
|---|---|
| `ExtNavTrafficDataSource` | 外部ナビ API ライブラリの traffic client を呼び、中立 DTO に詰める |
| `TrafficOverlayManager` | 案内開始 / 経過時間 / 走行距離 / reroute を見て取得し、route 近接フィルタを適用する |
| `NewGuidanceManager` | guidance lifecycle に合わせて `TrafficOverlayManager` を start / stop する |
| `MapViewModel` | `trafficOverlayState` を公開する |
| `MapEffect` | `TrafficOverlayState.incidents` から `MapCallOutRequest` を作る |

`TrafficOverlayManager` は `ExtNavGuidanceTracker.snapshot` を読んで現在累積距離や走行距離を判断してよい。
...
area 取得の初期実装では、過剰取得を避けるため最短間隔を 60 秒以上にする。pathCode 取得へ切り替えた
後は 10-15 秒程度の短い間隔を検討できるが、実測負荷と API 利用条件を確認してから決める。
...
将来 `TrafficOverlayState` が最新渋滞 segment を持つようになった場合は、MapEffect 側で
「動的 overlay があれば優先、無ければ `RouteDetail.congestionSegments`」の順に読む。
...
3. **B: 規制・事故 CallOut**
   ...
   - `TrafficOverlayState` を `GuidanceProgress` とは別に配る。
...
| B の polling 間隔 | area 取得では 60 秒以上、pathCode 取得では 10-15 秒を候補にする | 実装 PR と実測 |

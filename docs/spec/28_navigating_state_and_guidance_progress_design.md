...
- spec/18 (`18_external_api_migration_plan.md`) が外部API 移行の全体計画
...
- 音声案内・リルート検知・到着判定・カメラ制御は `ExtApiProgressSnapshot` を消費できる
- GPS の生位置は `NewGuidanceManager` が `ExtApiGuidanceTracker` に投入し、ルート追従・発話・リルート判定は
...
| 項目 | 決定 |
|---|---|
| `GuidanceState` に進捗を載せる方式 | `Guiding` を `data class Guiding(route: RouteDetail, progress: GuidanceProgress)` 化。UI には現在 route と `GuidanceProgress`、Tracker 内部と周辺機能には `ExtApiProgressSnapshot` を使う |
...
- **GP (案内ポイント)**: 外部API ライブラリの案内テキスト発火点（`RouteGuidance.guidancePoints[]`）
...
- **オフルート候補**: 生 GPS と projection のズレ、進行方向、精度から「ルート外の可能性が高い」と判定された tick。
  これ自体はリルート確定ではなく、`ExtApiRerouteDetector` の debounce 入力
- **発話スロット**: GP と phrase をキーにした音声案内の発話単位。`ExtApiAnnouncementScheduler` が発話済み状態を持つ
...
`ExtApiRouteDataSource.kt` を読むと、ルート探索（Preview）時点で外部API ライブラリの
`GuidanceClient.resolveGuidance()` と `RouteClient.search()` を並列に叩いて、各候補の
`RouteGuidance` を `ExtApiRouteRegistry` に既にキャッシュ済み。ナビ進捗計算に必要な
polyline / 交差点 / GP / phrase / summary / congestion はすべてここから取れる。

通常案内中の位置 tick では、

- **ROUTE / GUIDE のどちらも追加で叩かない**
- Tracker は Registry から `ExtApiRoutePayload.routeGuidance` を読み、位置 tick のたびに
  オンメモリで進捗を再計算するだけ
- Tracker 自体はネットワーク I/O を持たない

リルート時は、`ExtApiRouteDataSource.searchRoutes(currentLocation, destination, remainingViaPoints)`
を再呼び出しして新しい `RouteGuidance` を取得する。

一方、渋滞・規制情報は時間経過で古くなるため、案内中に `ExtApiTrafficOverlay` が別経路で更新する。
更新トリガは以下:
...
### 1.1 既存 Preview フロー（`ExtApiRouteDataSource.searchRoutes`）の確認
...
RouteRepository.searchRoutes
   │
   ▼
ExtApiRouteDataSource.searchRoutes (origin, destination, intermediates)
   ...
   │
   │       registry.put(ExtApiRoutePayload(id = routeId, routeGuidance = routeGuidance))
   │
...
再利用され、古い `RouteDetail` と新しい payload の取り違えが起きうる。`ExtApiRouteDataSource.searchRoutes`
は検索開始時に `registry.beginSession()` を呼び、旧 payload を clear したうえで、その session id を
全候補の `RouteDetail.id` / `ExtApiRoutePayload.id` に含める。
...
`ExtApiRoutePayload.routeGuidance: RouteGuidance` から `ExtApiGuidanceMapper` 経由で取り出す:
...
| `routeGuidance.guidancePoints[].phrases[]` | 音声フレーズ / マニューバ種別判定 | `ExtApiAnnouncementScheduler`, `GuidanceManeuverInfo` |
| `routeGuidance.guidancePoints[].phrases[].category` | `GuidanceCategory`（マニューバ種別の射影元） | `ManeuverType`, `ExtApiAnnouncementScheduler` |
...
| `routeGuidance.congestionSegments[]` | ルート探索時点の初期渋滞 snapshot。`ExtApiTrafficOverlay` の最新値が無い場合の fallback | `CongestionSegment` |
...
- **Tracker の責務**: 渋滞セグメントを所有しない。必要な UI / 音声は
  `ExtApiProgressSnapshot.currentCumulativeMeters` と traffic overlay の最新セグメントから前方渋滞を派生する
- **動的更新**: 交通情報の再取得は `ExtApiTrafficOverlay` の責務。案内開始直後 / 5 分経過 /
...
`ExtApiRouteDataSource.buildCongestionSegments(...)` で `CongestionSegment` 中立モデルに射影済み:
...
| `source` | `GUIDANCE_POINT / ROUTE_LINK`（外部API 側の算出元） |

初期 snapshot / 動的 overlay のどちらも `GuidanceProgress` には逐次再計算では入れない。理由:

- polyline 色分けは `RouteDetail.congestionSegments` または `ExtApiTrafficOverlay` のセグメントを
...
現在位置 (`ExtApiProgressSnapshot.currentCumulativeMeters`) より先の最初の congestion を抽出する。
`GuidanceProgress` に `upcomingCongestion` を足すのは、UI が常時必要とすることが確定した場合に限る。

#### 1.4.3 ナビ中の動的更新

spec/18 §3.2 の `ExtApiTrafficOverlay` は、Tracker とは別の状態として扱う:

```
ExtApiTrafficOverlay
...
`GuidanceState.Guiding.progress` と `ExtApiTrafficOverlay` を組み合わせて描画する。
...
**一律 scaleFactor は使わず**、既存 `ExtApiRouteDataSource.buildRoadClassSegments(...)` と同じ考え方の
アンカー付き距離 mapper を使う:
...
O(log n) で次 GP を引ける。`RouteDistanceMapper` は `core/navigation/extapi` 内の小さな純 Kotlin
 ...
 * `ExtApiGuidanceTracker` の `snapshot.progress` として生成される UI 用モデル。
 */
@Immutable
...
必要とする projection 生データは、下記 `ExtApiProgressSnapshot` に分けて保持する。
...
### 2.1.1 `ExtApiProgressSnapshot`（Tracker 内部 / 周辺機能用）
...
```kotlin
@Immutable
data class ExtApiProgressSnapshot(
...
| `isOffRouteCandidate` | `ExtApiRerouteDetector` 入力。UI には直接出さない |
...
| `ExtApiAnnouncementScheduler` | phrase ごとの明示的な発話距離 / 発話タイミング。GP は距離を持つが、1 GP 内の複数 phrase が同じタイミングか段階発話かは型定義に依存する | `GuidancePoint.distanceFromStartMetres`, `GuidancePoint.phrases[]` | `gpIndex + phraseIndex + category` を発話済みキーにし、明示的な trigger が無い phrase は GP までの残距離と速度から標準スロットに割り当てる | `Phrase` に trigger distance / audio id / ssml text 相当があるか確認 |
...
- 案内開始直後（初回位置取得前）は `progress = null` を許容せず、`lastKnown()` または
  `ExtApiGuidanceBootstrap.fromOrigin(...)` で非 null の `Guiding(route, progress)` を即時公開する（§3.5.4）
...
### 2.5 `ExtApiRoutePayload`（既存）

`core/navigation/src/androidMain/.../extapi/ExtApiRouteDataSource.kt` の **末尾 (L1028-1032)
に既に定義済み**。再掲のみ:

```kotlin
@Immutable
data class ExtApiRoutePayload(
    val id: String,
    val routeGuidance: RouteGuidance,
)
```

`RouteGuidance` は外部API ライブラリの guidance domain 型。
**OneNavi 側コードからは `ExtApiGuidanceMapper` を経由する場合のみ参照**し、他クラスは
直接ライブラリ型を持ち出さない（命名ポリシー + 抽象化）。

`ExtApiRouteRegistry` は route id の衝突を避けるため、検索ごとに session を切る:

```kotlin
class ExtApiRouteRegistry {
    fun beginSession(): String {
        clear()
        return newSessionId()
    }

    fun put(payload: ExtApiRoutePayload)
    fun get(routeId: String): ExtApiRoutePayload?
    fun clear()
}
```

`ExtApiRouteDataSource.searchRoutes(...)` は `beginSession()` の戻り値を route id に含める。
`RouteDetail.id` と `ExtApiRoutePayload.id` は常に同じ値にする。
...
├── extapi/
│   ├── ExtApiGuidanceTracker.kt
│   ├── ExtApiRerouteDetector.kt
│   ├── ExtApiAnnouncementScheduler.kt
│   ├── ExtApiVoicePlayer.kt
│   ├── ExtApiGuidanceMapper.kt             # internal
│   ├── RouteDistanceMapper.kt              # summary 距離 → geometry 距離のアンカー付き変換
│   ├── ExtApiRoutePayload.kt
│   └── (既存) ExtApiRouteRegistry.kt, ExtApiRouteDataSource.kt 等
└── newguidance/
...
class CurrentLocationDataSource(
     ...
     * 取得できた場合は即座に 1 tick 分の進捗算出に使い、駄目なら `ExtApiGuidanceBootstrap`
...
- 案内中の GPS tick は `NewGuidanceManager` だけが購読し、各 tick を `ExtApiGuidanceTracker.onLocation(...)`
...
### 3.3 `ExtApiGuidanceTracker`

```kotlin
class ExtApiGuidanceTracker {
    /** 現在の projection snapshot。null = 未 attach */
    val snapshot: StateFlow<ExtApiProgressSnapshot?>
     ...
     */
    fun attach(payload: ExtApiRoutePayload, route: RouteDetail)
...
4. **残時間の補正**
   ...
   - 動的交通情報がある場合は `ExtApiTrafficOverlay` が残時間補正値を提供し、UI 側で組み合わせる
...
6. **内部 snapshot の保持**
   ...
   - 音声・リルート・到着判定が必要にする `projectionErrorMeters` / `vehicleSpeedMps` /
     `currentCumulativeMeters` などは `ExtApiProgressSnapshot` に残す

#### 3.3.2 GPS 入力とルート追従判定

`ExtApiGuidanceTracker` は GPS tick の正規化点。`UserLocation` を受け取り、ルート geometry 上の
projection として `ExtApiProgressSnapshot` を更新する。Tracker は **リルート開始や音声再生を直接行わない**。
...
### 3.4 `ExtApiGuidanceMapper`（`internal`）

外部API ライブラリの型 → OneNavi モデル の変換は **必ずここに集約**。他クラスはライブラリ型を持ち出さない。

#### 3.4.1 入力構造

`ExtApiRouteDataSource.kt` で参照されている既存ライブラリ型の事実を整理:
...
これは `ExtApiRouteDataSource.kt` の `hasHighwaySign` 判定と同じロジック。

#### 3.4.6 API シグネチャ

```kotlin
internal object ExtApiGuidanceMapper {
    /**
...
既に `ExtApiRouteDataSource.kt` で参照されているため、本ドキュメントでも同じ表記を使う。
事業者名・製品名・パッケージ root のドメイン名は引き続き露出させない。

`GuidancePoint.phrases[].category` の具体的な列挙値・`Phrase` の正確な型・`lane` /
`directionSigns` 等のフィールドの存在は外部API ライブラリ側の定義を正とする。未確定の項目は
...
class NewGuidanceManager(
    private val registry: ExtApiRouteRegistry,
    private val tracker: ExtApiGuidanceTracker,
    private val rerouteDetector: ExtApiRerouteDetector,
    private val announcementScheduler: ExtApiAnnouncementScheduler,
    private val locationDataSource: CurrentLocationDataSource,
    private val guidanceBootstrap: ExtApiGuidanceBootstrap,
            ...
            try {
                tracker.snapshot.collect { snapshot ->
                    if (snapshot != null) {
                        ...
                        if (decision is ExtApiRerouteDecision.Request) {
    ...
    private suspend fun handleReroute(request: ExtApiRerouteDecision.Request) {
...
- Tracker が生成した `ExtApiProgressSnapshot` を `GuidanceProgress` 更新、リルート判定、音声発話、到着判定へ fan-out する
...
- `ExtApiRerouteDetector` は §3.6、`ExtApiAnnouncementScheduler` は §3.7、
  `ExtApiGuidanceBootstrap` は §3.8 で定義する

### 3.6 `ExtApiRerouteDetector`（`internal`）

`ExtApiRerouteDetector` は `ExtApiProgressSnapshot` の連続 tick からリルート開始条件を確定する。
Tracker は 1 tick ごとの `isOffRouteCandidate` を出すだけで、連続性・cooldown・目的地直前の抑制は
Detector が持つ。

```kotlin
internal class ExtApiRerouteDetector {
    fun attach(route: RouteDetail)
    fun onSnapshot(snapshot: ExtApiProgressSnapshot): ExtApiRerouteDecision
    fun detach()
}

internal sealed interface ExtApiRerouteDecision {
    data object None : ExtApiRerouteDecision
    data class Request(
        val origin: RoutePoint,
        val destination: RoutePoint,
        val remainingViaPoints: ImmutableList<RoutePoint>,
        val currentCumulativeMeters: Double,
        val reason: ExtApiRerouteReason,
    ) : ExtApiRerouteDecision
}

internal enum class ExtApiRerouteReason {
    OffRoute,
}
```
...
### 3.7 `ExtApiAnnouncementScheduler`（`internal`）

`ExtApiAnnouncementScheduler` は GPS 位置に応じた音声発話タイミングを制御する。入力は
`ExtApiProgressSnapshot` と `RouteGuidance.guidancePoints[].phrases[]`。GPS Flow を直接 collect せず、
Tracker の projection snapshot を唯一の時刻・距離基準にする。

```kotlin
internal class ExtApiAnnouncementScheduler(
    private val voicePlayer: ExtApiVoicePlayer,
) {
    fun attach(payload: ExtApiRoutePayload, route: RouteDetail)
    fun onSnapshot(snapshot: ExtApiProgressSnapshot)
    fun detach()
}

internal interface ExtApiVoicePlayer {
    fun enqueue(announcement: ExtApiAnnouncement)
    fun clear()
}

@Immutable
internal data class ExtApiAnnouncement(
    val guidancePointIndex: Int,
    val phraseIndex: Int,
...
`ExtApiTtsVoicePlayer` は `ExtApiVoicePlayer` の具象実装。音声合成・AudioFocus・キュー排他を持ち、
...
### 3.8 `ExtApiGuidanceBootstrap`（`internal`）

初回 GPS tick 前にも UI を出すための「ルート始点ベース progress」を作る小さなヘルパ。

```kotlin
internal class ExtApiGuidanceBootstrap {
    fun fromOrigin(
        payload: ExtApiRoutePayload,
        route: RouteDetail,
...
val navigationModule: Module = module {
    ...
    single { ExtApiRouteRegistry() }
    single<RouteDataSource> { ExtApiRouteDataSource(...) }
    // ... 略

    // Guidance
    single { ExtApiGuidanceTracker() }
    single { ExtApiRerouteDetector() }
    single<ExtApiVoicePlayer> { ExtApiTtsVoicePlayer(...) }
    single { ExtApiAnnouncementScheduler(voicePlayer = get()) }
    single { ExtApiGuidanceBootstrap() }
    single {
        NewGuidanceManager(
            registry = get(),
            tracker = get(),
...
> 作り直されて再度 `startGuidance` してもインスタンスは健全に動作する。`ExtApiGuidanceTracker` /
> `ExtApiRerouteDetector` / `ExtApiAnnouncementScheduler` / `ExtApiGuidanceBootstrap` も同様に `single`
...
MapViewModel.handleNavigationStart
   ...
   └─ scope.launch { newGuidanceManager.startGuidance(RouteDetail) }
        │
        ├─ ExtApiRouteRegistry.get(route.id) → ExtApiRoutePayload (Preview でキャッシュ済み)
        ├─ ExtApiGuidanceTracker.attach(payload, route)
        │    └─ GP 累積距離テーブル事前計算 (§1.5 の RouteDistanceMapper で射影)
        ├─ ExtApiRerouteDetector.attach(route)
        ├─ ExtApiAnnouncementScheduler.attach(payload, route)
        │
        ├─ ★初期化分岐:
...
[1st 位置 tick (FusedLocation から)]
   │
ExtApiGuidanceTracker.onLocation(userLocation)
   ...
   └─ snapshot.value = ExtApiProgressSnapshot(progress = GuidanceProgress(...), ...)
   │
   ▼
NewGuidanceManager snapshot fan-out
   ├─ ExtApiRerouteDetector.onSnapshot(snapshot)
   │    └─ Request なら GuidanceState.Rerouting → routeRepository.searchRoutes(...) → tracker.attach(newPayload, newRoute)
   ├─ ExtApiAnnouncementScheduler.onSnapshot(snapshot)
   │    └─ 発話スロット到達なら ExtApiVoicePlayer.enqueue(...)
   ├─ 到着判定
...
| `ExtApiGuidanceTrackerTest` | attach → onLocation で `snapshot` が更新される / `GuidanceProgress` と内部 projection 値が整合する / 単調増加が維持される / 投影誤差が 5m 以下 / 次 GP 切り替わり |
| `ExtApiRerouteDetectorTest` | off-route candidate 3 tick 連続で `Request` / 1 tick だけなら `None` / 低精度 GPS・停止中・目的地直前は抑制 / cooldown 中は再要求しない |
| `ExtApiAnnouncementSchedulerTest` | GP までの距離スロット到達で発話 enqueue / 同じ `(gpIndex, phraseIndex, category)` は二重発話しない / off-route・rerouting・arrived 中は発話しない / 速度に応じた先読み・遅延 |
| `ExtApiGuidanceMapperTest` | 外部ライブラリのモックデータから `GuidanceManeuverInfo` / `LaneGuidance` / `DirectionSign` / `HighwayPanel` が正しく生成される |
| `ExtApiRouteRegistryTest` | `beginSession()` で旧 payload が clear される / session id を含む route id だけ取得できる |
...
| コンポーネント | 入力 | 出力 / 反映先 |
|---|---|---|
| `ExtApiAnnouncementScheduler` | `ExtApiProgressSnapshot`, `RouteGuidance.guidancePoints[].phrases[]`, `RouteDistanceMapper` で射影した GP 累積距離 | 発話キュー。既発話判定には `guidancePointIndex + phraseIndex + category` を使う |
| `ExtApiVoicePlayer` | `ExtApiAnnouncement` | 音声合成 / 再生キュー / AudioFocus |
| `ExtApiRerouteDetector` | `projectionErrorMeters`, `isOffRouteCandidate`, `rawLocation`, `currentCumulativeMeters` | `ExtApiRerouteDecision.Request`、`GuidanceState.Rerouting`、再探索要求 |
| `RouteRepository` | `ExtApiRerouteDecision.Request` の origin / destination / remainingViaPoints | reroute 後の `RouteDetail` と `ExtApiRoutePayload` |
| 到着判定 | `distanceRemainingMeters`, `currentCumulativeMeters`, 目的地近傍判定 | `GuidanceState.Arrived` |
| `ExtApiCameraController` | `snappedLocation`, `bearingDegrees`, `matchedSegmentIndex`, `nextManeuver` | 地図カメラの bearing / zoom / tilt |
| ForegroundService | `CurrentLocationDataSource.locationUpdates()` | バックグラウンド継続時の位置購読 |
| `ExtApiTrafficOverlay` | route path code 群、`RouteDetail.congestionSegments` | 動的渋滞セグメント、規制情報 overlay |
...
`GuidanceProgress` は UI 表示に必要な安定モデル、`ExtApiProgressSnapshot` は周辺コンポーネント向けの
...
- ライブラリ: 「外部API ライブラリ」
- クラス prefix: `ExtApi`
- BuildKonfig / 環境変数: `EXT_API_*`

外部ライブラリの実型名（`RouteGuidance` / `GuidancePoint` / `Intersection` / `GuidanceCategory` 等）は
ドメインを表す一般名なので本ドキュメント上で参照しても OK（既存 `ExtApiRouteDataSource.kt` でも
...
| 初回 GPS tick 前で `lastKnown()` なし | `ExtApiGuidanceBootstrap.fromOrigin(...)` の progress を `Guiding(route, progress)` として即時公開 |
...
- `docs/spec/18_external_api_migration_plan.md` — 外部API 移行計画（正本）

...
| 項目 | 決定 |
|---|---|
| `GuidanceState` に進捗を載せる方式 | `Guiding` を `data class Guiding(progress: GuidanceProgress)` 化。UI には `GuidanceProgress`、Tracker 内部と周辺機能には `ExtNavProgressSnapshot` を使う |
...
| UI モデル | 足りない / 未確定のデータ | 候補 / 既存の取得口 | 未確定時の扱い | 必要な確認 |
|---|---|---|---|---|
...
- 案内開始直後（初回位置取得前）は `Guiding(progress=null)` ではなく、`lastKnown()` または
  `ExtNavGuidanceBootstrap.fromOrigin(...)` で非 null の `Guiding(progress)` を即時公開する（§3.5.4）
        ...
        scope.launch(session) {
            try {
                tracker.snapshot.collect { snapshot ->
                    if (snapshot != null) {
                        _state.value = GuidanceState.Guiding(snapshot.progress)
                    }
                }
            } catch (cancellation: CancellationException) {
                throw cancellation
...
MapViewModel.handleNavigationStart
   ...
   └─ scope.launch { newGuidanceManager.startGuidance(RouteDetail) }
        ...
        │      ├─ 取得成功 → tracker.onLocation(it)  → tracker.snapshot 更新 → Guiding(progress)
        ...
        └─ scope.launch(session) { try { tracker.snapshot.collect { _state = Guiding(it.progress) } }
...
| コンポーネント | 入力 | 出力 / 反映先 |
|---|---|---|
| `ExtNavAnnouncementScheduler` | `ExtNavProgressSnapshot`, `RouteGuidance.guidancePoints[].phrases[]` | 発話キュー。既発話判定には `nextGuidancePointIndex` と phrase 情報を使う |
| `ExtNavRerouteDetector` | `projectionErrorMeters`, `isOffRouteCandidate`, `rawLocation`, `currentCumulativeMeters` | `GuidanceState.Rerouting`、再探索要求 |
...
| 初回 GPS tick 前で `lastKnown()` あり | `tracker.onLocation(lastKnown)` の結果を `Guiding(progress)` として公開 |
| 初回 GPS tick 前で `lastKnown()` なし | `ExtNavGuidanceBootstrap.fromOrigin(...)` の progress を `Guiding(progress)` として即時公開 |

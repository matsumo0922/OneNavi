...
試行錯誤の末に確立した「外部ナビ API ライブラリのルートを Google Routes API
...
外部ナビ API ライブラリが返す polyline (= LineString) と origin/destination
...
- spec/18 (外部ナビ API 移行計画), spec/20 (NavigationView 反射注入調査),
  spec/21 (外部ナビ proto / announcement 仕様), spec/22 (route token / Custom
  Navigator 評価) の通読を推奨。本ドキュメントは spec/22 §11 の実機 curl 検証
  で出た「形状追従できない」結論を **waypoint 選定戦略を改善することで一定
  程度克服できる** ことを示し、その手順を残す。
- **N 社の API 提供事業者名・製品名は本ドキュメント上に出さない**
  (CLAUDE.md §厳命)。本ドキュメントでは「外部ナビ API ライブラリ」「外部
  ルート」「N 社」の代替表記のみ使用する。
...
「外部ナビ API ライブラリが返したルート」と「Google Routes API v2 を chunk 分割
...
`drive-supporter-api/analysis/sample/<sample-id>/extracted/` 以下の生データ:
...
で `drive-supporter-api/analysis/sample/...` を直接 fetch している。これにより
...
`drive-supporter-api/analysis/sample/<id>/raw/mocha-route.json` の `unit.datum`
...
なお実装側 `drive-supporter-api/drive-supporter-api/src/main/kotlin/.../core/net/
DriveSupporterHeaders.kt` は `x-up-datum: wgs84` を全リクエストに付与済みなので、
...
外部ナビ API が返す GP (= guide point / 案内地点) には大別して 2 種ある:
...
つくば市内が 0 になる原因: NAVITIME GP は「案内が発生する地点」を中心に記録
...
| layer | 色 | 内容 |
|---|---|---|
| External polyline | 青 #1e88e5 | 外部ナビ API (datum 変換後) |
...
ユーザ click 強制 waypoint のロジックは、本番側で **「外部ナビ API が返した
特定の重要地点 (例: ユーザが行先候補として選んだ POI、ピン留めした地点) を
必ず経由する Routes API 呼び出し」** が必要になった時に再利用できる
(§11.2.3 `ExtNavRouteRefiner` に `forcedWaypoints` 引数を追加する形)。
...
実装側 (drive-supporter-api/) では生 proto から取るので別途 `decodeSjis()`
が必要 (`drive-supporter-api/CLAUDE.md` §文字コード参照)。
...
```
drive-supporter-api/analysis/sample/shakuji-tsukuba/extracted/route/route-polyline.geojson
```

tokyo-gotemba / tokyo-nagoya-hiroshima / hiroshima-ferry-beppu には `extracted/
route/` 自体が無く GP-based fallback 動作を確認できない。`drive-supporter-api/
...
|---|---|---|
| §2 datum 変換 | **不要** | 実装側 `DriveSupporterHeaders` で `x-up-datum: wgs84` を全リクエストに付与済み。ライブラリ経由 polyline は WGS84 |
...
外部ナビ API ライブラリの中核ロジックは別管理のプライベートリポジトリ
(`drive-supporter-api/`) に閉じている。Android 側との接合は、Map 表示の
ための polyline を返す関数として行う:

```
[drive-supporter-api] (private repo)
   ├ RouteResponse (内部)
...
[OneNavi composeApp / feature/navigation]
   └ ExtNavRouteRefiner          ← 本ドキュメントで定義する Kotlin 実装
...
```kotlin
suspend fun refineExtNavRoute(
    extPolyline: List<LatLng>,
    origin: LatLng,
    destination: LatLng,
): List<LatLng> {
    val intermediates = ExtNavRouteRefiner.samplePolylineWaypoints(extPolyline)
    val all = listOf(origin.toWaypoint()) + intermediates + listOf(destination.toWaypoint())
    val chunks = ExtNavRouteRefiner.chunkWaypoints(all)
    return ExtNavRouteRefiner.computeChunkedRoute(chunks, useVia = true)
}
```
...
2. **Routes API クライアント** (`core/datasource/routes/`): Ktor + BuildKonfig 経由
   API key
3. **`ExtNavRouteRefiner`** (`feature/navigation/`): §11.2.3 のクラス群
4. **polyline decoder**: `core/common/` に置く (Mapbox SDK 統合時にも流用可能)
5. **画面側統合**: `feature/home/` のルート表示で外部ライブラリ polyline ではなく
   refined polyline を Mapbox に渡す
...
- shakuji-tsukuba の extPolyline を fixture として持ち、`ExtNavRouteRefiner`
...
`drive-supporter-api/drive-supporter-api/src/main/kotlin/.../guidance/` 配下に
...
- **proto フィールド名の機械生成依存**: `unknown_104`, `field_3.field_1` 等は
  ...
  あるはず。drive-supporter-api/ 側で proto schema を整備すれば名前が安定する。
...
5. Inflation が悪い / waypoint が偏っているなら §4-§6 のロジックを順に検証:
   ...
   - via / heading 設定の確認
6. 検証スクリプトは python で `drive-supporter-api/analysis/sample/<id>/

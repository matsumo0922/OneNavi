...
### 8.3 チューニングのお勧め値 (shakuji-tsukuba 検証時)

- 案内地点 = None
- 案内地点外 = Every 4th (約 19 picks)
- Chunk size = 25
- Set heading = ON
- Use via waypoints = ON

これで polyline inflation が 1.0x 付近 (= 外部ルートとほぼ完全一致)、つくば
section にも waypoint が散る。
...
現状アルゴリズムは TypeScript で dev-tool に閉じている。本番 (Android Navigation
統合) で使うには Kotlin に翻訳する必要がある。

落とし込み先候補:

- `composeApp` または新規 `feature/navigation/` モジュール内の
  `RouteWaypointBuilder` 相当
- 入力: 外部ナビ API ライブラリ (`drive-supporter-api`) の `RouteResponse` /
  `GuidanceResponse`
- 出力: Routes API リクエスト用 waypoint 列 (`com.google.maps.routing.v2.Waypoint`
  相当の構造)

具体的なクラス分割案:

```
ExtNavRouteToGoogleWaypoints
├ ManeuverDetector       (§3): 案内地点 index 集合を作る
├ StructuralPointDetector (§4): excludeFromBetween 集合を作る
├ PolylineSampler         (§5): 外部 polyline からの FPS sampling
├ ChunkedRoutesApiClient  (§7): chunk 分割 + リクエスト + decode
└ WaypointBuilder         (統合): heading + via 設定を含む waypoint 配列を組む
```

`drive-supporter-api/drive-supporter-api/src/main/kotlin/.../guidance/` 配下に
ある `GuideProtoMapper` と機能が近いので、そちらを再利用 / 拡張する形が良い。
...
→ 本番統合時は両者を比較する benchmarking が必要。spec/22 D-2211 を更新する
価値あり。判断材料:

...
> **作成日:** 2026-04-22
> **ステータス:** ドラフト（レビュー待ち）
> **対象:** ルート検索・turn-by-turn 案内源を **外部API**（外部API 提供元の某カーナビアプリを参考にした第三者実装のライブラリ / 別管理のプライベートリポジトリ）に移譲し、地図描画は Google Maps SDK (NavigationView) を維持する方針への大規模計画変更

> **用語:** 本ドキュメントでは実在事業者名・製品名を直接書かず、当該事業者を「**外部API 提供元**」、当該ライブラリを「**外部API ライブラリ**」と総称する。API 仕様・解析資料・認証情報はすべて当該プライベートリポジトリ側に閉じ込め、本リポジトリ（OneNavi）側には実名・エンドポイント・プロトコル詳細を持ち込まない。
...
「旧地図/ナビ provider → Google Routes + 自前 guidance」移行までを前提としていた。
今回、別管理のプライベートリポジトリで **外部API** を叩く Android ライブラリを自前で
仕上げ、合法性リスクを自己責任の範囲に閉じ込めた上で「ルート検索 / turn-by-turn 案内 /
方面看板・交差点画像 / 交通情報」を **外部API 提供元相当の外部API** に委譲する方向へ舵を切る。

本ドキュメントは以降の実装計画の**正本**となり、これ以前の旧 provider / 自前音声テンプレ
...
| レイヤ | 担当 | 備考 |
|---|---|---|
| ルート検索（候補取得・料金・所要時間・距離） | 外部API `RouteClient.search()` | ルート検索エンドポイント（JSON）|
| turn-by-turn 案内データ（音声テキスト・交差点・方面看板・GP シーケンス・ポリライン） | 外部API `GuidanceClient.resolveGuidance()` | 案内バイナリエンドポイント（protobuf）|
| 案内画像（青看板・高速 3D・料金所） | 外部API `GuideImageClient.preload()` | 画像 fixdata エンドポイント |
| 交通情報（渋滞・規制・事故） | 外部API `TrafficClient` | 交通情報エンドポイント群 |
...
| 音声案内の発話 | 自前 TTS エンジン（Android TTS / Google Cloud TTS Chirp 3 HD） | テキスト組み立ては外部API の SSML をそのまま利用 |
| ナビ状態マシン・セッション管理 | 自前（`GuidanceSessionManager` の書き換え） | Navigator には依存しない |
| リルート判定・到着判定・経由地通過判定 | 自前 | 外部API 再検索を自前で走らせる |
...
- `NavigationView` は通常の `MapView` より
  ...
  を標準で持つ。これらは外部API 側に無く、自前で再実装すると追加コストが大きい。
- Navigator（`setDestinations` / `startGuidance`）だけ使わないことで、
  **「ルーティング/turn-by-turn は外部API、描画は Google」** という分担が成立する。
...
### 1.3 「Google Navigation SDK に外部API のルートを食わせない」理由
...
1. **`CustomRoutesOptions.setRouteToken()` は Google Routes API 発行の routeToken 限定**。
   外部API のポリラインを直接食わせる API は存在しない。
...
6. routeToken 使用時であっても、ルートは Google が "best matching guideline" として扱い、
   **完全追従は保証されない**（公式明記）。外部APIのポリラインに厳密に沿わせることは
   Google の設計上不可能。
...
|---|---|---|
| 次ターンバナー（矢印 + 距離 + 交差点名） | `TOP_START` | 外部API `Guidance.guidancePoints[next].intersectionName` + distance remaining |
| レーンガイダンス | `TOP_START` 配下 | 外部API `GuidancePoint.laneMarkers[]` |
| 方面看板（DirectionSign） | `TOP_END` | 外部API `GuidancePoint.directionSigns[]` |
| 高速パネル（IC/JCT/SA/PA） | `TOP_END` 配下 or 独立レイヤ | 外部API `intersections[]` + `roadName` |
| ETA / 残距離 / 残時間 | `BOTTOM_END` | `ExtApiGuidanceTracker` の進捗計算結果 |
| 速度計 + 速度制限 | `BOTTOM_START` | 現在地の速度 + 外部API `GuidePoint.speedLimit` |
| リルート中インジケータ | `TOP_START` 被せ | `ExtApiRerouteDetector` の状態 |
...
```kotlin
setField("EXT_API_LOGIN_ID")
setField("EXT_API_PASSWORD")
```
...
「自分が持つ外部API の契約情報を local.properties に書く」形式を強制する。外部API
...
外部API ライブラリ側の `deviceUuid` は毎回ランダム発番するとサーバー側でアノマリー扱い
される疑いがあるため、**初回起動時に生成し DataStore に保存**する。`AppSetting` /
`AppSettingDataSource` に `extApiDeviceUuid: String?` を追加し、null なら新規生成し
保存する。
...
| `TurnByTurnUpdateBus` / `NavigationUpdatesService` / `NavigationFeedSnapshot` / `NavigationStepSnapshot` / `NavigationLaneSnapshot` | **削除**。SDK からの NavInfo に依存しない構造にする。代わりに `GuidanceFeed`（外部API 由来）を定義する。 |
...
| `core/model/GuidanceEvent` / `DistanceBucket` / `FollowupDistanceBucket` / `FollowupManeuver` / `ManeuverType` / `ManeuverModifier` / `LaneInfo` / `LanePosition` / `StraightforwardLevel` / `GuidancePhrase` / `TtsPhraseId` / `CompassDirection` | **ほぼ全削除**。外部API の SSML をそのまま読み上げるため、マニューバ分類・距離バケット・フレーズ ID などは不要になる。UI に必要な `ManeuverType` / `ManeuverModifier` だけは「現在 GP の方向アイコン」を決める用途で残す（外部API の guidance category や approach angle から射影する）。 |
| `core/datasource/GoogleRoutesDataSource` | Phase 1 は残し、Phase 2 で完全削除（後述）。 |
| `core/model/RouteResult.platformRoute: Any?` | **削除**（`Any?` のまま外部API の Route を突っ込まない）。Android 側の feature で型安全に外部API の `Route` / 自前 `ExtApiRoute` を保持する。既存の Home リファクタ計画 12 の方針を徹底する。 |
| `core/model/GoogleRoute` の `routeToken` | **削除可**。Navigator に渡さないため不要。`GoogleRoute` 自体も「Google 由来」という意味が薄れるので `ExtApiRoute`（External API Route）にリネームする。 |

### 3.2 新規導入

| 新規クラス | 配置 | 責務 |
|---|---|---|
| `ExtApiRoute` | `core/navigation/androidMain` | 外部API の Route + Guidance のペア。UI に渡す単位。|
| `ExtApiRouteRepository` | `core/repository`（ただし実装は androidMain） | 外部API の `RouteClient.search()` + `GuidanceClient.resolveGuidance()` を並列呼び出しして `ExtApiRoute` の候補リストを返す |
| `ExtApiRouteDataSource` | `core/datasource/androidMain` | ライブラリのファサード（`ExtApiClient` 相当）を注入。認証ゲートを経由してルート検索する |
| `ExtApiAuthGateway` | `core/navigation/androidMain` | 起動時に `AuthClient.currentState()` を読み、SignedOut なら `signInWithCredentials(AppConfig)` を実行。失敗時は UI にエラー表示 |
| `ExtApiGuidanceTracker` | `core/navigation/androidMain` | `Guidance.guidancePoints` + 現在地（`RoadSnappedLocationProvider`）を入力とし、最近傍 GP を判定して進捗を算出する |
| `ExtApiAnnouncementScheduler` | `core/navigation/androidMain` | `ExtApiGuidanceTracker` の進捗から「この GP の `phrases[]` のどれを今発話すべきか」を決定し、SSML を TTS に渡す |
| `ExtApiRerouteDetector` | `core/navigation/androidMain` | 現在地がルート形状から閾値以上離れたら `onOffRoute()` を発火。一定時間後に `ExtApiRouteRepository.resolveGuidance(current → goal + 残経由地)` で再検索 |
| `ExtApiTrafficOverlay` | `core/navigation/androidMain` | `TrafficClient.listByPathCodes(...)` を経路に紐付くコードで定期ポーリング（Phase 2） |
| `ExtApiGuideImagePreloader` | `core/navigation/androidMain` | ルート確定時に `GuideImageClient.preload(guidance)` を走らせ、画像を Coil ディスクキャッシュに落とす |
| `ExtApiSsmlSpeaker` | `core/navigation/tts` | 既存 `TtsEngine` をラップ。外部API の SSML 内に含まれる独自 phoneme 拡張を、発話対象 TTS で読める形に射影する（詳細 §7）|
...
HomeMapViewModel.onRouteSearch
  │
  ▼
ExtApiRouteRepository.search(criteria)
  ...
  │
  └─ 候補ごとに ExtApiRoute(route = routes[i], guidance = guidance[i], imagePreloadHandle)
  │
  ▼
RouteManager.setRoutes(extApiRoutes)
  │
  ▼
UI:
  ├─ 地図: 各候補のポリラインを自前で描画
...
HomeMapViewModel.onNavigationStarted
  │
  ▼
GuidanceSessionManager.startSession(selected: ExtApiRoute)
  ├─ RoadSnappedLocationProvider を購読開始（NavigationApi 初期化は app 起動時）
  ├─ ExtApiGuidanceTracker.attach(route, location)
  ├─ ExtApiAnnouncementScheduler.attach(tracker)
  ├─ ExtApiRerouteDetector.attach(route, location)
  ├─ ExtApiSsmlSpeaker.startSessionGreeting()
...
ExtApiSsmlSpeaker
  ...
  │
  ▼
ExtApiRerouteDetector
  ├─ offRouteDistance > threshold が N 秒継続 → onOffRoute()
  ├─ 即座に ExtApiSsmlSpeaker で "ルートから外れました" を発話（CRITICAL）
  └─ ExtApiRouteRepository.search(criteria=currentLoc→goal+remainingViaPoints) を実行
    ├─ Guidance 差し替え
    ├─ ExtApiGuidanceTracker を新ルートに attach し直す
    └─ 発話マーク（spoken）を全クリア
  │
  ▼
Arrival 判定
  ├─ 現在地が目的地 < 50m で `ExtApiSsmlSpeaker.speak("まもなく目的地です")`
...
外部API の案内バイナリエンドポイント（protobuf）は `priority` 系クエリで 1 つのルートしか返さない（ROUTE1 固定）。代替ルート 3 つを表示したい場合は **priority を変えて 3 回叩く** のが自然な解。
...
- 結果 `GuideImage[]` を **Coil の MemoryCache + DiskCache** に `ext-api-image-<minor>` キーで保持
- UI で表示するときは `rememberAsyncImagePainter(key="ext-api-image-<minor>")` で参照
...
| Search | Google Places | 現状維持（外部API の spot 検索は Phase 4 以降の判断） |
| RoutePreview | Google Routes 結果を 3 本表示 | 外部API の結果を 1〜3 本表示 |
| ActiveGuidance | Navigator 主導 | ExtApiGuidanceTracker / ExtApiAnnouncementScheduler 主導 |
...
外部API のネイティブ SDK 専用バイナリ（ROUTE 系）は自前解析不可。したがって
...
外部API のポリラインと完全一致しないため、ルート線と自車 chevron がズレる瞬間が発生する。
...
外部API の地図スタイルを揃える検討をする（現状、旧 provider の studio 相当のカスタムは Google
...
外部API が返す `SsmlPhrase.ssml` は以下のような形（独自 phoneme 拡張あり）:
...
1. **Android 内蔵 TTS**: SSML を一応受け付けるが独自 alphabet は動かない。
   ...
   読み間違いは外部API の意図した読みと異なる可能性があるが、漢字読みは TTS 側に委ねる。
...
いずれにせよ **発話タイミング・発話テキストは外部API が決める**。OneNavi 側が持つのは:
- 発話キューと優先度制御
...
300 / 100 / 50 / 0）。ExtApiAnnouncementScheduler は以下ロジックで発火:
...
外部API が対応していない境界フレーズ（「ルート検索中です」「ルート案内を開始します」
...
- 外部API ライブラリを composite build で取り込み、ビルドが通る状態にする
- `ExtApiAuthGateway` / `ExtApiRouteDataSource` / `ExtApiRouteRepository` を実装
...
- `GuidanceSessionManager` / `GuidanceCoordinator` / `GuidancePlanner` / `PhraseComposer`
  を新実装に置き換え（既存は削除）
- `ExtApiGuidanceTracker` / `ExtApiAnnouncementScheduler` / `ExtApiRerouteDetector` の初版実装
- `HomeMapsMapEffectContent` のポリライン描画を外部API 由来に差し替え
...
- ユニットテスト: 既存 `GuidancePlannerTest` / `SpeechDispatcherTest` を廃止し、
  `ExtApiGuidanceTrackerTest` / `ExtApiAnnouncementSchedulerTest` を追加
...
- Android Auto 対応
- 外部API 依存を前提にした高速パネルの Auto ページネーション（6 件制限）
...
| リスク | 影響 | 対処 |
|---|---|---|
| 外部API 事業者の利用規約違反 | 個人開発であっても BAN / 法的リスク | 本実装は**利用者個人の契約を用いた自己利用のための API クライアント**であり、credential を公開しない。OSS としては「コードのみ」公開、事業者との契約は利用者自身が個別に結ぶことを README に明記。公開ドキュメントに事業者名・製品名・エンドポイント URL を書かない。第三者実装は事業者のサポート対象外であり、利用規約の範囲は利用者が確認する前提。 |
...
| 外部API のデータ更新頻度 | 道路変更に追随しない可能性 | `header.json.M-Format_Version` を監視し古いキャッシュを捨てる運用 |
...
| 外部API の Geocoding / Autocomplete を使わない | 目的地検索の日本語品質 | 現状の Google Places を継続。検索は外部API に委譲しない |

---

## 10. 決定ログ (D-100〜)

### D-100: ルート検索・turn-by-turn を外部API に移譲
- **Date:** 2026-04-22
- **Context:** 別管理のプライベートリポジトリに auth/route/guidance/image/traffic 5 feature を揃えた
  Android ライブラリが実 API で動作確認済み。音声案内の品質が旧 provider / 自前テンプレ / Google Routes を大幅に上回る。
- **Decision:** 採用。D-001 での「外部API 不採用」を上書き。個人利用・自己責任範囲での
...
### D-102: 旧地図/ナビ provider フルスタック（D-002）は破棄
- **Date:** 2026-04-22
- **Decision:** 旧地図/ナビ provider への依存は撤去する。`local.properties` の旧 provider token も削除対象。
- **Rationale:** Google + 外部API の分担で全要件をカバーできる。

### D-103: 自前日本語テンプレート（D-003）も破棄
- **Date:** 2026-04-22
- **Decision:** 通称辞書・信号機案内・合流案内テンプレート・速度連動タイミング・自然な日本語
  テンプレートの自前構築は全て撤回。外部API の SSML をそのまま読み上げる。
- **Rationale:** 外部API が既に「完璧」なテキストを返すため、自前実装は冗長。

### D-104: 音声エンジンの選択（D-004）は維持
- **Date:** 2026-04-22
- **Decision:** Google Cloud TTS Chirp 3 HD（Laomedeia）をメイン、Android 内蔵 TTS を
  フォールバック、という構成は維持する。ただし入力テキストの組み立ては外部API 側に委譲。
- **Rationale:** Chirp 3 HD の音声品質は変わらず優秀。自前テンプレートが消えるだけで、
  エンジンは同じ。

### D-105: 交差点拡大図（Junction View）は外部API の案内画像で代替
- **Date:** 2026-04-22
- **Decision:** `GuideImageClient.preload` による高速 JCT 3D 分岐イラスト、一般道交差点の
  青看板を直接表示する。旧 provider の Junction View 日本対応検証は不要になる（Q-001 クローズ）。
- **Rationale:** 外部API が返す 3D JCT 画像は商用品質。自前プロシージャル生成より優秀。

### D-106: SA/PA データの同梱（D-005）は不要になる
- **Date:** 2026-04-22
- **Decision:** `assets/sapa.json` / OSM Overpass API による事前抽出パイプラインは不要。
  外部API の案内に SA/PA が含まれる。
- **Rationale:** 自前 DB 管理の保守コスト削減。

### D-107: 外部API credential の配置
- **Date:** 2026-04-22
- **Decision:** `local.properties` の `EXT_API_LOGIN_ID` / `EXT_API_PASSWORD` を BuildKonfig
...
- **Decision:** 本リポジトリ（OneNavi）の公開ドキュメント・README・コミットメッセージ・
  クラス名・識別子・コメントには、外部API の事業者名・製品名・エンドポイント URL を
  一切記載しない。総称として「外部API」「外部API 提供元」を用いる。プロトコル詳細・解析資料は
...
- **Status:** UNRESOLVED
- **Question:** 外部API 側で `deviceUuid` は 1 端末 = 1 UUID を期待しているのか、
...
T01. Gradle 設定: composite build で外部API ライブラリ取り込み（パスは gradle property）
T02. BuildKonfig / AppConfig / local.properties に credential 追加
T03. DeviceUuid 永続化（AppSetting / AppSettingDataSource 拡張）
T04. 外部API ファサードを Koin DI に登録（設定オブジェクト組み立て含む）
T05. ExtApiAuthGateway 実装 + ensureSignedIn() を RouteDataSource のエントリで呼ぶ
T06. ExtApiRouteDataSource / ExtApiRouteRepository 実装（priority=recommended 固定）
T07. ExtApiRoute / GoogleRoute 削除 / RouteItem 互換層
...
T09. GuidanceSessionManager を ExtApiGuidanceTracker / ExtApiAnnouncementScheduler 対応に書き換え
...
T11. ExtApiGuidanceTracker 実装（最近傍 GP / 残距離 / 残時間算出）
T12. ExtApiAnnouncementScheduler 実装（phrase.distanceMetres ベースの発火）
T13. ExtApiSsmlSpeaker 実装（SSML プレーン化 → 既存 TtsEngine に投入）
T14. ExtApiRerouteDetector 実装（offRoute 判定 + 再検索）
T15. ExtApiCameraController 実装（§1.5 の zoom/tilt/bearing ロジック + manual gesture 一時停止）
T16. 自前 TBT UI（Compose）実装: 次ターンバナー / レーン / ETA カード / 速度計 / 方面看板
T17. NavigationView.setCustomControl 配線（§1.4 の overlay 位置表通り）
T18. HomeMapsMapEffectContent のポリライン描画を外部API 由来（sub_path_samples）に差し替え
...
T23. 既存旧 provider 関連コード・local.properties 上の旧 provider token 撤去（Phase 1 末に実施）
```

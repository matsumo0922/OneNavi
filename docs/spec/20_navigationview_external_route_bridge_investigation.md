...
2026-04-23 時点で、POC の最初の骨格までは実装済み。

- `NavigationViewReflectionBridge` は `NavigationView` attach 時に `vd.f` / `bb.e` を掴み、guidance 開始時に `vd.f.a()` を呼ぶ
- 同 bridge から lower seam の `bb.e.a(bv.h)` / `bb.e.b()` を呼べるようにした
- route overlay payload は `NavigationViewSyntheticRouteOverlayFactory` で synthetic に組み立てる
- 現在の synthetic route は `aw -> az -> be -> bv.h` の最小構成で、`ca` は default `kg`、`ij/hp` は validation を通す最低限の field だけを反射で埋める
- `GuidanceSessionManager` からは guidance 開始時と route 更新時に route list を bridge へ流すように配線済み

この段階の狙いは **まず built-in route line / alternative route overlay が lower seam だけで出るか** を観測することにある。  
まだ `vd.f.i(to.a, prev)` は未実装で、CallOut / 交差点自動拡大は次段階の対象である。

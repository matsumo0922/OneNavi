...
> **対象:** Google Navigation SDK `NavigationView` を、Google 側でルート探索を行わず、**外部API** のルート/案内データだけで駆動できるかの技術調査
> **関連:** `16_turn_by_turn_navigation_flow.md`, `17_callout_redesign.md`, `18_external_api_migration_plan.md`
...
つまり、本件は「Google の navigator をだます」のではなく、**Google の map/navigation UI controller が期待する内部モデルを外部API から再構築する**問題だと整理できる。
...
- ルート探索は外部API が行う
- リルートも外部API が行う
- alternative routes 取得も外部API が行う
...
- 代わりに `RoadSnappedLocationProvider` から map-matched 位置を受け、`ExtApiGuidanceTracker` で進捗・次 maneuver・off-route 判定を自前計算している
- `ExtApiRouteDataSource` は外部API で得たルートを `GoogleRoute` に浅く射影しているが、`routeToken` は `null`、`steps` は空
...
4. 外部API の `Guidance` / `GuidancePoint` / `Intersection` / dense polyline を、その内部 model に写像できるか
...
- **公開 API のみ**で外部API のルートと step を標準 UI に流し込むことはできない
...
- 外部API から来た brand-new route を後から 1 本だけ注入する
...
外部API 側には dense polyline が既にあるため、最も実現しやすい。
...
現状の `ExtApiRouteDataSource` は 1 本固定だが、internal 側の model 自体は複数ルート対応になっている。
...
既存の `ExtApiGuidanceTracker` は dense polyline 上への投影と進捗 m の算出ができているため、このロジックを step anchor 計算にも再利用できる。
...
- SDK 内部には lane 表現がある
- しかし外部API の現在の public domain model では lane marker が表に出ていない
...
一方で `bq.o` が実際に要求しているのは「画像 ID」そのものより **距離 along route + decoration kind** に近く、外部API の `imageRefs` をそのまま 1:1 で写せる保証はない。
...
幸い、現行の `ExtApiGuidanceTracker` は polyline 射影を既に持っているため、新たに geometry engine を起こす必要はない。
...
### 9.1 `ExtApiInternalRouteMapper`

責務:

- 外部API の route/guidance を synthetic `az` 群へ変換する
...
### 9.3 `ExtApiNavigationUiBridge`

責務:

- `ExtApiProgressSnapshot` から current step / remaining distance/time を更新する
...
今の `ExtApiRouteDataSource` は 1 本固定で `steps` も空。
...
### 10.3 Step 3: `ExtApiInternalRouteMapper` を作る
...
- `ExtApiProgressSnapshot` を bridge へ渡す
...
| class | 役割 |
|---|---|
| `ExtApiInternalRouteMapper` | 外部 route/guidance -> synthetic `az` / `be` 変換 |
| `ExtApiInternalStepMapper` | `GuidancePoint` / `Intersection` -> `bk` / `bl` 変換 |
| `NavigationViewReflectionBridge` | `NavigationView` の private seam 呼び出しを隔離 |
| `ExtApiUiStateMapper` | `ExtApiProgressSnapshot` -> `rf.b` / `sm.h` / `to.a` 変換 |
| `ExtApiBridgeSession` | `NavigationView` 1 インスタンスに対する old/new state 管理 |

### 11.2 既存 class の変更候補

| class | 変更内容 |
|---|---|
| `ExtApiRouteDataSource` | 複数 route 取得、route set registry 化 |
...
2. `ExtApiRouteDataSource` を複数 route 対応へ広げる
3. `ExtApiInternalRouteMapper` / `NavigationViewReflectionBridge` を debug-only で実装する

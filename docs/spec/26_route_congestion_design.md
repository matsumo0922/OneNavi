...
> **作成日:** 2026-05-13
> **更新日:** 2026-05-13（Phase 0 = 外部ナビ API ライブラリ側の RE 完了。`field 12` 確定）
...
> **関連:** `19_drive_supporter_api_integration_plan.md`, `21_ext_nav_guide_proto_and_announcement.md`, `27_route_congestion_visualization_design.md`（UI / 音声側）
...
| Phase | 場所 | 内容 | 状態 |
|---|---|---|---|
| **P0** | drive-supporter-api | 渋滞中ルートをキャプチャして `GUIDE` バイナリの渋滞構造を RE。`field 12 = CongestionEnvelope { CongestionAttrEx, TrafficInfo }` を確定 | **完了 (2026-05-13)** |
| **P1** | drive-supporter-api | (a) `RouteSearchCriteria.useTrafficInfo: Boolean = true` 追加 + `DsrQueryBuilder` で `traffictime` + `trafficsearch:1` を emit (b) `guide.proto` に確定スキーマ反映 (c) `GuideProtoMapper` に渋滞パース + 隣接 GP 集約実装 (d) fixture テスト (oizumi-choshi / keiyo-funabashi / tokyo-yokohama の DSR ZIP) | 着手前 |
| **P2-a** | OneNavi | `core/model` 拡張（`CongestionSegment` 拡張、`CongestionTrend` 追加、`RouteDetail.congestionSegments` 追加） | **完了** |
| **P2-b** | OneNavi | `ExtNavRouteDataSource` で `RouteGuidance.congestionSegments` を中立モデルに配線 | **完了**（P1 待ちで実データは空） |

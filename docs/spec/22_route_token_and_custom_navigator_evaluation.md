...
- 結果として「**任意ルート (= 外部ナビ API が計算した polyline) に対応する token を発行する**」という仮説の前提が、公開実装・decompile の双方で破綻した。
...
> 経由で外部ナビ API のルートをそのまま案内させることができる。
...
- 「外部ナビ API が計算した polyline をそのまま token 化する」公式手段は無い。
- 仮に Routes API で waypoints を投げて token を取っても、それは Google 側のルートであって N 社のルートではない。
...
- `ExtNavInternalRouteMapper` で synthetic `az` / `be` を構築
...
「外部ナビ API のルートをそのまま案内する」目的では使えない。
...
外部 API の guidance points には音声 only の情報案内 (合流地点 / レーン減少 / 交差点接近など) が混ざっており、これを waypoints として渡す必要はない。サンプル DSR データ (drive-supporter-api/analysis/sample/) で実測した結果:
...
ところが N 社の guidance points に含まれる物理マニューバには次のような **本線通過 / pass-through ノード** が混ざる:
...
| **Google 計算ルートが N 社ルートを忠実再現できるか** | ❌ 距離 50% 膨張、max 6.9km の経路逸脱 |
...
これは spec/20 の反射注入路線が回避できる問題 (= 自前で N 社の polyline をそのまま `bb.e.a(bv.h)` に流せる) のため、**形状忠実性の観点では反射注入路線が依然優位**。

### 11.7 結論の更新

- **D-2209 (2026-04-30):** routeToken + chunk 分割アーキテクチャは、(a) 物理マニューバフィルタによる waypoint 削減で chunk 数は実用範囲、(b) 公式 API での UI/audio 抑制と reroute 奪取は可能、にもかかわらず (c) Google の stopover 制約により実ルートと **50% の距離膨張** が発生し、地図描画レイヤーの主目的 (= N 社ルートの忠実描画) を達成できない。本路線は採用不可と確定する。
...
> もし 25 を超えて waypoint を渡せれば、N 社の polyline を細かく辿る Google ルートを計算させ、
...
- N 社 polyline 全長 (例 100 km) を 25 stopover ずつ刻むと leg 数が爆発し、毎区間 token 取得 = Routes API 課金の山
...
stopover の数 (= arrival announcement 連発) を諦めて、**25 stopover を慎重に選んで N 社 polyline の主要屈曲点だけ並べる**案も検討に値する。しかし:

- N 社 polyline が Google の道路ネットワークに存在しない裏道や接続を選んでいる場合、25 stopover で誘導しても Google は **道路ネットワーク制約に従って別経路を返す**。形状近似が成立しない。
- N 社の代表的優位点 (信号回避抜け道, 動的トラフィック判断) はそもそも Google ルートエンジンの挙動と異なるため、stopover 経由でも忠実再現できない。
- 仮に幾何が似たとしても、リルート時に Navigator は token 内 objectives で **再計算** するため、走行中に N 社ルートから乖離していく。
...
したがって「外部ナビ API のルートを Google routeToken 発行で近似して Navigator に走らせる」路線は:
...
- **D-2207 (2026-04-30, 訂正済):** Routes Preferred 路線は **技術的に成立** するが、(a) "select customers only" の個別契約ハードル (OneNavi 個人 OSS では取れない)、(b) 全 waypoint stopover による中間 arrival announcement 挙動の未検証懸念、(c) Google 道路網依存と objectives 再計算による N 社ルートからの乖離、で OneNavi 現状では採用しない。本線は spec/20 の反射注入のまま。

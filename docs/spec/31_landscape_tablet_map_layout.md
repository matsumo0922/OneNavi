...
│       ├─ MapItem    (地図, fillMaxSize)   ← haze source は従来どおりここ（§9.7）
...
7. **haze**: 現状 `hazeState` の consumer は未配線（`MapNavigationManeuverPanelSection` が
   `@Suppress("UnusedParameter")`）。本設計では haze をリスク対象から外し、source（地図）を従来どおり
   渡すのみ。将来 consumer を配線する際に SurfaceView 越えの可否を別途確認（Codex Finding 2 / Claude 1-C）。
...
| haze の consumer 未配線 | MED | リスク対象外・現状パススルー（§9.7） |

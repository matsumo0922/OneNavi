...
外部ナビ API ライブラリに委譲する。turn-by-turn の進捗追跡・callout 表示・音声案内も
自前で行う。
...
- **v2 (現行)**: Google Navigation SDK の `Navigator` を使った案内 (route_token + chunk
  ...
  callout / 音声案内を自前で持つ方式に変更。外部ナビ API ライブラリは元々デコード済みの
  polyline と GP / 交差点 / maneuver データを返すため、Routes API による polyline 再現
  (旧 `ExtNavRouteRefiner` 系) は不要になった。
- **v1 (廃止)**: spec/23 の Routes API + waypoint sampling アルゴリズムで外部ルートを
  ...
  案内する方式。`ExtNavRouteRefiner` / `RoutesApiClient` / `DefaultRoutesApiClient` /
...
| ルート探索 | 外部ナビ API ライブラリ → `ExtNavRouteDataSource` → `RouteRepository`。デコード済み polyline + GP / 交差点 / maneuver データを返す |
| ルート描画 | `GoogleRoute.geometry` を自前 `MapPolyline` で描画 |
| 走行進捗 / リルート検知 / 到達検知 | **自前**。生 GPS (`FusedLocationProviderClient`) を polyline に投影して算出する (`ExtNavGuidanceTracker` 系) |
| callout / 音声案内 | 外部ナビ API ライブラリ由来の GP テキストを進捗にトリガーして自前で発話 (`ExtNavAnnouncementScheduler` + `tts/*`) |
...
- **外部ナビ API ライブラリ**: 別管理のプライベートリポジトリに存在する N 社 API の
  ラッパライブラリ。本リポジトリ (OneNavi) からは公開 interface 経由で参照する。
  本ドキュメントは Kotlin 統合側のみ扱う
- **N 社**: 外部ナビ API 提供事業者の代替表記。具体名は本ドキュメント・コード上に一切
  露出させない (`CLAUDE.md` §厳命)
- **GP (案内ポイント)**: 外部ナビ API ライブラリの案内テキスト発火点。"およそ 500m 先
  三軒寺左方向です" のようなテキストと発話距離を持つ
- **進捗投影**: 生 GPS 位置を polyline 上の最近接セグメントに射影し、単調増加な走行距離 /
  残距離 / 次 GP / 次交差点 / 次 maneuver を算出する処理 (`ExtNavGuidanceTracker`)
...
│       │                  │ ExtNavRerouteDetector + TTS  │    │
│       │                  └──────────────────────────────┘    │
└───────┼──────────────────────────────────────────────────────┘
        ▼
┌──────────────────────────────────────────────────────────────┐
│ core/navigation                                              │
│   ExtNavRouteDataSource (RouteDataSource impl)               │
│     └─ 外部ナビ API ライブラリ (別管理プライベート repo)     │
│          - ルート探索のみ                                     │
...
[2. NewRouteManager.searchRoutes(waypoints)]
    ↓  state = Searching
[3. 外部ナビ API ライブラリにルート探索を問い合わせ (RouteRepository.searchRoutes)]
...
[7. 「案内開始」ボタン → NewGuidanceManager.startGuidance(selectedRoute)]
    ↓  state = Guiding
[8. Guidance 中 (今後実装)]
    │   - 生 GPS を ExtNavGuidanceTracker で polyline に投影
    │   - 進捗に応じて GP テキストを TTS で発話、callout を更新
    │   - polyline からの逸脱を ExtNavRerouteDetector で検知
    ↓
[9a. 目的地到達 → state = Arrived]
[9b. 逸脱検知 → 外部ナビ API ライブラリで再探索 → 新 geometry に差し替え (state = Rerouting → Guiding)]
```
...
├── extnav/                          # 外部ナビ API ライブラリ統合層 + 自前進捗/音声の部品 (既存)
│   ├── ExtNavRouteDataSource.kt     # RouteDataSource 実装。GoogleRoute.geometry を構築
│   ├── ExtNavRouteRegistry.kt       # GoogleRoute.id → ExtNavRoutePayload (route + guidance)
│   ├── ExtNavGuidanceTracker.kt     # polyline 投影による進捗追跡 (Guidance 接続予定)
│   ├── ExtNavAnnouncementScheduler.kt  # GP テキストの発話スケジューラ (同上)
│   ├── ExtNavRerouteDetector.kt     # polyline 逸脱検知 (同上)
│   ├── ExtNavSsmlSpeaker.kt
│   ├── ExtNavAuthGateway.kt / ExtNavClientProvider.kt
...
> 現状 `ExtNavRouteDataSource` は候補 1 本固定 (priority=Recommended)。複数候補化は外部ライブラリ
...
`ExtNavRouteRegistry` から `ExtNavRoutePayload` (route + guidance) を引き、
`ExtNavGuidanceTracker.attach(guidance)` + `FusedLocationProviderClient` の位置更新を購読し、
進捗に応じて `ExtNavAnnouncementScheduler` で GP を発話、`ExtNavRerouteDetector` で逸脱検知、
...
MapViewModel: pushScreenState(RoutePreview(waypoints, ...)) ; NewRouteManager.searchRoutes(waypoints)
   │  state = Searching
   ▼
RouteRepository.searchRoutes() → ExtNavRouteDataSource → 外部ナビ API ライブラリ
...
NewGuidanceManager.startGuidance(route: GoogleRoute)
   ├ ExtNavRouteRegistry.get(route.id) → ExtNavRoutePayload (route + guidance)
   ├ ExtNavGuidanceTracker.attach(payload.guidance)   # polyline / 交差点 / GP の累積距離を前計算
   ├ FusedLocationProviderClient で位置更新を購読 → tracker.onLocation(lat, lng)
   ├ ExtNavAnnouncementScheduler を tracker.state に接続 (GP 到達で発話)
   ├ ExtNavRerouteDetector を位置更新に接続
   └ state = Guiding
```

`ExtNavGuidanceTracker` / `ExtNavAnnouncementScheduler` / `ExtNavRerouteDetector` は既に
...
- 進捗 (走行距離 / 残距離 / 残時間 / 次 GP / 次交差点 / 次 maneuver): `ExtNavGuidanceTracker.state`
- 音声: `ExtNavAnnouncementScheduler` が GP の発話距離に達したら `ExtNavSsmlSpeaker` 経由で発話。
  CRITICAL カテゴリは throttle をバイパス
- callout / マニューバパネル: `tracker.state` を UI が購読して描画 (Compose)

### 5.3 リルート

`ExtNavRerouteDetector` が「polyline からの垂直距離 > 80m が 5 秒継続」で `onOffRoute()` を発火
→ `state = Rerouting` → `RouteRepository.searchRoutes(currentLocation, destination)` で再探索
→ 新 `GoogleRoute` の `geometry` で polyline を差し替え、`ExtNavGuidanceTracker.attach` をやり直し
→ `state = Guiding`。SDK の自動リルートは存在しない (Navigator を使わないため) のでチラつきも無い。

### 5.4 到達

`ExtNavGuidanceTracker` の残距離が閾値以下で到達とみなす (外部ライブラリの `summary` も参照)
→ `state = Arrived`。
...
| GPS 信号喪失 (Guidance) | `ExtNavGuidanceTracker` は最後の投影点を保持。復帰時に再投影 (今後実装) |
...
- `ExtNavGuidanceTrackerTest` (既存): polyline 投影・進捗計算
...
### 9.1 N 社名露出禁止 (`CLAUDE.md` §厳命)

本ドキュメント・実装ファイル中で N 社の事業者名・製品名・パッケージ名・ドメイン名を
**一切露出させない**。代替表記: 事業者 = 「N 社」、ライブラリ = 「外部ナビ API ライブラリ」、
クラス prefix = `ExtNav`、BuildKonfig = `EXT_NAV_*`。
...
1. **Guidance の中身**: `NewGuidanceManager` を `ExtNavGuidanceTracker` / `ExtNavAnnouncementScheduler` /
   `ExtNavRerouteDetector` / `tts/*` + 生 GPS (`FusedLocationProviderClient`) に接続する (§5)
2. **複数ルート候補**: `ExtNavRouteDataSource` の候補 1 本固定を解除 (外部ライブラリ側の対応と合わせて)

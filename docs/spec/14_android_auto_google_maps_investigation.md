...
> 2026-06-06 更新:
> 現行作業ツリーでは `CarAppService` の manifest 公開は戻っており、
> `androidx.car.app.NAVIGATION_TEMPLATES` と `androidx.car.app.ACCESS_SURFACE` も宣言済みである。
> ただし turn-by-turn の `NavigationManager.updateTrip()` 連携や route preview / search / settings の
> template 構成はまだ入っておらず、Android Auto ナビとしては最小実装段階である。
...
- `NavigationTemplate`
- `Action.PAN`
- `AppManager.setSurfaceCallback()` による app-owned surface の取得
- `VirtualDisplay` + `Presentation` 上の `MapView` 描画
- 交通情報 / 3D 建物 / 現在地レイヤーの有効化
- `SurfaceCallback.onClick()` / `onScroll()` / `onScale()` の最低限の処理
- host の visible area に応じた map padding と Compose overlay padding
- Compose 製の地図コントロール overlay
...
- `composeApp/src/androidMain/kotlin/me/matsumo/onenavi/car/OneNaviCarMapRenderer.kt`
- `feature/map/src/androidMain/kotlin/me/matsumo/onenavi/feature/map/car/CarMapControlsOverlay.kt`

### 6.2 現在の実装が最小構成であること

`OneNaviCarMapScreen` は `NavigationTemplate.Builder()` を返し、ActionStrip と `Action.PAN` だけを設定している。  
現時点では turn-by-turn のテンプレート更新、トリップカード、代替ルート、検索、設定パネルなどは未実装である。

`OneNaviCarMapRenderer` は、Android Auto host から渡された `Surface` を `VirtualDisplay` の出力にし、
その `Presentation` 上へ Google Maps SDK の `MapView` と Compose overlay を載せている。
`SurfaceCallback` からの click / scroll / scale は最低限処理されており、交通情報、3D 建物、現在地レイヤーも有効化している。

一方で、ルートライン、代替ルート、POI オーバーレイ、Follow / Overview / Pan のカメラ状態管理、
`NavigationManager.updateTrip()` 連携はまだ入っていない。

つまり OneNavi は「Android Auto ナビの器」と「map surface 描画の初期基盤」はできているが、
Google Maps が見せているようなリッチさを構成するナビ状態管理と route / POI 表現はまだ未着手である。
...
2026-06-06 時点の `composeApp/src/androidMain/AndroidManifest.xml` では、両 permission と navigation category 付きの `CarAppService` が宣言済みである。

もし今後 OneNavi 側で独自の map surface 制御を明示的に進めるなら、以下は再確認すべきである。

- `ACCESS_SURFACE` を manifest に宣言しているか
- 利用する地図 / ナビ SDK が内部で要求するライフサイクルや host 条件を満たしているか
...
AppManager.setSurfaceCallback() で受け取る Surface
  ...
  -> FrameLayout(MapView + ComposeView)
  -> setContentView(container)
  -> MapView と ComposeView { OneNaviNavigationOverlay(state) }
```
...
3. **地図レンダラ（Google Maps SDK）の Surface 制約**
   現状の地図は `play-services-maps` ベースである。`MapView` は `FrameLayout` を継承する View であり、
   任意の `Surface` を直接渡す公開 API は見当たらない。
   そのため Android Auto の `Surface` へ地図を直接流すのではなく、公式の Draw maps サンプルと同じく
   `VirtualDisplay` + `Presentation` 上の View 階層として `MapView` を載せるのが現実的である。
   `MapNavigationContent` を**地図ごと**単一 `ComposeView` に載せる構成は成立しにくいため、
   地図は `MapView`、上物は**透過 `ComposeView`**
...
2. 透過 `ComposeView` を `VirtualDisplay` + `Presentation` で Android Auto Surface に出す最小 PoC
   ...
   2026-06-06 時点では、`MapView` + Compose の View 階層を `Presentation` に載せる初期実装までは入っている。
3. その上に `ManeuverPanel` だけを仮 state で載せる。
4. `GuidanceManager` の state を car session に配線する。
5. DHU / 実機で `Surface` 取得、地図描画、タッチ転送、visible area padding を検証する。
...
公式資料と現行リポジトリを突き合わせた結果、本文の主張は概ね妥当である。

- **確認できたこと**
  - Android for Cars App Library の map 系 template は host が提供する `Surface` に app が地図を描ける。
  - `Surface` 取得には template permission とは別に `androidx.car.app.ACCESS_SURFACE` が必要である。
  - `AppManager.setSurfaceCallback()` で得た `Surface` には、`Canvas` 直接描画だけでなく `VirtualDisplay` + `Presentation` による View 描画も公式サンプルで示されている。
  - `NavigationTemplate` は drawing surface を直接露出せず、描画には `setSurfaceCallback()` を使う。
  - `SurfaceCallback` は click / scroll / scale / fling / visible area / stable area などを提供し、map action strip に `Action.PAN` を置くことで pan / zoom callback を受け取る構造である。
  - active navigation のメタデータは `NavigationManager.updateTrip()` で host に渡す。
  - Car App Library 1.7.0 stable では `MapWithContentTemplate` が中心で、`MapTemplate` / `PlaceListNavigationTemplate` / `RoutePreviewNavigationTemplate` は deprecated である。

- **確認できなかったこと**
  - Google Maps 本体が実際にどの private / internal API を使っているかは公開資料からは確認できない。
  - したがって本文の Google Maps 実装分担は「公開 API で説明できる実装モデルからの推論」として扱うべきであり、「Google Maps が必ずこの実装である」という断定ではない。

- **現行 OneNavi との差分**
  - `composeApp/src/androidMain/AndroidManifest.xml` は `NAVIGATION_TEMPLATES` / `ACCESS_SURFACE` と navigation category の `CarAppService` を持っている。
  - `OneNaviCarMapScreen` は `NavigationTemplate` と `Action.PAN` を返し、`AppManager.setSurfaceCallback()` に `OneNaviCarMapRenderer` を登録している。
  - `OneNaviCarMapRenderer` は `VirtualDisplay` + `Presentation` 上に Google Maps SDK の `MapView` と Compose overlay を描画している。
  - `NavigationManager.updateTrip()`、route line、代替ルート、沿道検索、settings template はまだ未実装である。
...
4. `composeApp/src/androidMain/kotlin/me/matsumo/onenavi/car/OneNaviCarMapRenderer.kt`
5. `composeApp/src/androidMain/AndroidManifest.xml`
6. `gradle/libs.versions.toml`
7. `feature/map/src/androidMain/kotlin/me/matsumo/onenavi/feature/map/car/CarMapControlsOverlay.kt`

...
┌──────────────────────────── OneNavi Android ────────────────────────────┐
│ Route search UI                                                        │
│   └─ GuidanceDataSource ── ServerGuidanceDataSource                    │
│        └─ GuidanceApiClient                                            │
│                                                                        │
│ ExtNav adapter ──> RouteDetail ──> Google Maps SDK                     │
...
| D14 | junction assetはroute確定時にprefetch可能な不透明URIにする | key秘匿と案内中ローカル駆動を両立 | アプリはroute確定直後にassetをcache。画像なしでも進行 |
...
data class RouteOptions(
    /**
     * 返してほしい候補ルート種別の集合。serverは各種別ごとに独立したHERE routing呼び出しを行い、
     * 結果を`RouteGuidance.priority`でラベル付けして`Guidance.routes`へ走行順位で格納する。
     * 既存の候補選択UI（推奨/高速優先/一般道優先タブ）を無改修で再現するための契約。
     */
    val requestedPriorities: Set<RoutePreference> = setOf(RoutePreference.RECOMMENDED),
...
| `POST /api/v1/guidance/reroutes` | `RerouteRequest` | top-level `Guidance` | off-route確定後に全再生成 |
| `POST /api/v1/guidance/traffic-refreshes` | `TrafficRefreshRequest` | top-level `Guidance` | 同一目的地をtraffic-awareで再評価。deltaではない |
...
- Android→serverの認証は、ローカル開発では無認証、本番相当では`X-OneNavi-Key`固定secretを任意採用する。
...
- `maxAlternativesPerPriority > 0`のときだけ種別内でHERE alternativesを要求する。種別をまたいだ同一geometryは§11.3.1の規則で重複排除し、要求集合の優先順でlabelを決める。
...
- wire modelへ外部URL/keyを露出せず、`/assets/junction/{assetId}`の不透明URIへ置換する。
- Androidは案内開始前にprefetchする。
...
```text
stable-id-input =
  modelVersion + routeGeometryFingerprint + kind + round(measureMeters * 10) + occurrence
stable-id = base32(sha256(stable-id-input))[0..19]
```

traffic refreshでgeometryが同一ならIDを維持する。rerouteでgeometryが変わった区間は新IDとし、未変更prefix区間はmeasure/geometry fingerprint照合で再利用してよい。
...
| `RouteGuidance.guidancePoints` : `ImmutableList<GuidancePoint>` | サーバ生成の安定ID | routeId + route measure(mm丸め) + kind + occurrenceをSHA-256短縮し、同一ルート再生成で安定化する。 | 外部プロバイダの一時IDをwire契約へ露出しない。 |
...
| `RouteGuidance.imageIds` : `ImmutableList<GuideImageRef>` | HERE junction viewアダプタ | 高速道路系decision pointに限定して取得し、route確定時にasset cacheへ取り込む。共有モデルには不透明asset URIを格納。 | 取得不可はnull/空。案内処理は画像なしで継続。 |
...
| `GuidancePoint.imageRefs` : `ImmutableList<GuideImageRef>` | HERE junction viewアダプタ | 高速道路系decision pointに限定して取得し、route確定時にasset cacheへ取り込む。共有モデルには不透明asset URIを格納。 | 取得不可はnull/空。案内処理は画像なしで継続。 |
...
| `Intersection.id` : `Int` | サーバ生成の安定ID | routeId + route measure(mm丸め) + kind + occurrenceをSHA-256短縮し、同一ルート再生成で安定化する。 | 外部プロバイダの一時IDをwire契約へ露出しない。 |
...
| `Intersection.imageRefs` : `ImmutableList<GuideImageRef>` | HERE junction viewアダプタ | 高速道路系decision pointに限定して取得し、route確定時にasset cacheへ取り込む。共有モデルには不透明asset URIを格納。 | 取得不可はnull/空。案内処理は画像なしで継続。 |
...
| `GuideAnnouncementBlock.id` : `String` | サーバ生成の安定ID | routeId + route measure(mm丸め) + kind + occurrenceをSHA-256短縮し、同一ルート再生成で安定化する。 | 外部プロバイダの一時IDをwire契約へ露出しない。 |
...
| `GuideAnnouncementBlock.groupId` : `Int` | サーバ生成の安定ID | routeId + route measure(mm丸め) + kind + occurrenceをSHA-256短縮し、同一ルート再生成で安定化する。 | 外部プロバイダの一時IDをwire契約へ露出しない。 |
...
| `RouteDetail.id` : `String` | サーバ生成の安定ID | routeId + route measure(mm丸め) + kind + occurrenceをSHA-256短縮し、同一ルート再生成で安定化する。 | 外部プロバイダの一時IDをwire契約へ露出しない。 |
...
既存OneNaviは1回の検索で複数の**名前付き候補ルート**（推奨/渋滞回避/高速優先/一般道優先/距離優先）を取得し、候補選択UIへタブで並べる。HEREの`alternatives`は意味ラベルを持たないため、これを**1つのpreferenceで表現することはできない**。よってrequestは`RouteOptions.requestedPriorities: Set<RoutePreference>`で「並べたい候補種別の集合」を受け、serverは各種別を**独立したHERE routing呼び出し**へ写像して結果を`RouteGuidance.priority`でラベル付けする。
...
- 得た候補を`RouteGuidance.priority`でラベル付けし、`Guidance.routes`へ格納する。既存の第一候補規約に合わせ、`RECOMMENDED`（無ければ要求集合の先頭）を`routes.first()`に置く。
- **同一geometryの候補は重複排除**する（例: `EXPRESS`と`RECOMMENDED`が同一経路になる場合、片方のlabelを残し1本に集約。どのlabelを残すかは要求集合の優先順で固定）。重複集約で候補数がUI期待より減ることはlogへ残す（§20.2、silent dropにしない）。
...
6. DI bindingをserver data sourceへ差替え。
7. route確定直後のjunction asset prefetchを既存image cache境界へ追加。
8. reroute/traffic refresh requestのcurrent/remaining points生成。
...
val routeDataModule = module {
    single<GuidanceApiClient> { KtorGuidanceApiClient(get(), get()) }
    single<GuidanceDataSource> { ServerGuidanceDataSource(get()) }
}
```

feature側は`GuidanceDataSource`だけをinjectする。build flavorやdebug menuでfixture/replayを切替える場合も、selector/bindingだけを変える。
...
`HttpClient`はDIで一度だけ生成し、Androidでは`OkHttp` engine、`ContentNegotiation { json(...) }`、`HttpTimeout`、固定secret header interceptorをinstallする。共有modelはkotlinx.serializationで直接decodeし、別のservice interface／converter層は追加しない。`KtorGuidanceApiClient`はこの`HttpClient`を薄く包み、URL組立・HTTP status mapping・serializer呼出だけを担当する。
...
| GAP-D11 | 複数名前付き候補ルート（推奨/高速優先/一般道優先等）の再現 | `requestedPriorities`集合 → 種別別HERE呼び出し → `RouteGuidance.priority`ラベル付け（§11.3.1） | `DATA-VALIDATION (OPEN)` | TODO-17。HEREに明示モードが無い種別の候補分化を実測するまで閉じない |
...
リスクを直列化しないため、稼働到達は次の順で刻む。各段は同じ`GuidanceDataSource`境界と共有wire model上で実現し、feature/UI/TTS/Google Maps消費層に段階判定を持ち込まない。

| 段階 | 新backendへ移す範囲 | 検証の中心 | rollback |
|---|---|---|---|
| S1 route基盤 | route、geometry、duration、tollをHEREベースで生成。block/ann/prefix/SSMLは当面`ExtApiGuidanceDataSource`を継続 | `RouteDetail`主要field、WGS84描画、ETA、料金UNKNOWN規則、route alignment | stage selectorを現行sourceへ戻す |
...
| S4 最終形 | reroute、traffic refresh、junction asset、全degradation/operationsを本設計どおり有効化 | 実機、長距離、failure injection、運用rollback | route単位で直前stageまたは現行sourceへ |

S1で異なるroute lineageのgeometryと案内点を無条件に混在させてはならない。合成する場合は`RouteAlignmentGate`で、origin/destination/via順一致、距離差3%以内、既存案内点の95%以上が新geometryの30m以内へ単調投影できることを確認する。不成立時は、そのtripのroute package全体を現行sourceへfallbackする。

...
OneNavi の現行方針は、地図表示を Google Maps SDK、経路・案内情報を外部ナビ API ライブラリ、補助的な経路再現や料金取得を Google Routes API に分担させる構成である。
...
- 外部ナビ API ライブラリから得た route geometry、渋滞区間、案内地点を overlay として表現できる。
...
**ルート検索、turn-by-turn 案内、交差点画像、交通情報は外部ナビ API ライブラリを primary source とする。**

### Rationale

- 案内品質と日本語表現を OneNavi 側で無理に再構築しない。
- 公開 repo には provider 実名・製品名・認証情報を露出させない。
- OneNavi 側は外部ナビ API ライブラリの抽象 model と Google Maps overlay に責務を限定する。
...
- dev-tools/route-compare による waypoint 選定検証
...
**案内テキストは外部ナビ API ライブラリ由来の情報を優先し、読み上げは Google Cloud TTS と Android 内蔵 TTS の二段構えにする。**
...
**渋滞、車線、方面看板、交差点画像は外部ナビ API ライブラリのデータを primary source とする。**

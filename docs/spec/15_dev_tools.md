...
HERE でルート検索すると、画面下部にドックするタブ式の詳細パネル (`route-detail.ts`) が出る。
HERE 固有データ（spans / tolls / snaps）を扱うため **HERE 専用**で、Google 選択時は表示しない。
provider は詳細表示用に `return`（actions / instructions / travelSummary / tolls）と `spans`
（length / duration / baseDuration / speedLimit / maxSpeed / dynamicSpeedInfo / functionalClass /
names）と `currency=JPY` を常時付与する（ユーザー選択と和集合）。

- **Summary**: 距離・所要（交通考慮）・base 所要・遅延・平均速度・transport・section/maneuver 数・
  料金合計。
- **Turn-by-turn**: `actions[]` を方位グリフ付きで順に表示（命令文・距離・所要）。
- **Graph**: `spans[]` から speed graph を canvas 描画。X=累積距離 (km)、系列=制限速度（灰破線）/
  base speed（青）/ traffic speed（ティール）を km/h で重ねる。
- **Snaps**: 各 waypoint の requested（`originalLocation`）vs snapped（`location`）と移動距離 (m)。
- **Tolls**: `tolls[]` の料金所ごとに料金システム・料金・支払方法、先頭に合計。
- **Response**: 生 JSON をダークなターミナル面に整形表示。

折りたたみ可。上端のハンドルをドラッグして高さを自由に調節でき、高さは `localStorage` に
永続化する。Clear / Google への切替で非表示にする。
...
└── src/
    ...
    ├── route-detail.ts      ← ルート詳細パネル (HERE・タブ式・speed graph)
    ├── providers/

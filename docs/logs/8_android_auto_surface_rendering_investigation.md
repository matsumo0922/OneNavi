...
4. **Google が滑らかなのは技術ではなく first-party 特権。** 後述の APK 解析（§実物証拠）のとおり、Maps は本物の Activity（`GhostActivity`）を projection display 上で動かす経路を使っており、生 Surface + semantic 入力に限定される公開側では構造的に同等にはできない。
...
- Google Maps（first-party）は、gearhead が signature permission 付きの本物の Activity（`GhostActivity`）を projection display 上に起動し、その Activity 内で Maps の通常 UI/renderer を動かしている（§実物証拠で APK 確認）。「生 Surface に全部自前描画」ではなく、本物の Window を持つ Activity を投影する構造。第三者用の Car App Library（template / 生 Surface / semantic 入力）とは別経路。
...
- 「Maps SDK を生 Surface へ直接描けば滑らかになる」→ 棄却。Maps SDK は `Activity` context 前提で生 Surface へ直接描画できない。
- 「Google Maps は生 Surface に生タッチが届く first-party 特権を持つ」→ 訂正。後述の APK 解析の結果、Google Maps はそもそも生 Surface を使っておらず、本物の Activity を projection している（メカニズムが異なる）。
...
## 実物証拠：Google Maps APK 解析

「Google Maps が Android Auto をどう描画/入力しているか」は公式ドキュメントに記述が無いため（第三者ドキュメント調査でも「不明」で確定）、APK を直接解析して事実を確認した。

- 解析対象: `com.google.android.apps.maps` `base.apk`（73.8MB, multidex 10 dex）。
- 手法: `apktool d -s`（manifest デコード）、`jadx --single-class`（個別クラス decompile）、`d2j-dex2jar`（クラス一覧）。
- 成果物: リポジトリ外 `APK/GoogleMap/cc_analysis/`（`REPORT_google_maps_android_auto.md`、`apktool_out/AndroidManifest.xml`、`jadx/sources/.../GhostActivity.java` 他、`base.jar`）。

### 確認した事実

1. **第三者用 Car App Library 経路は不在。** `AndroidManifest.xml` に `androidx.car.app.CarAppService` を action に持つ service は 0 件。`androidx.car.app.ACCESS_SURFACE` / `androidx.car.app.NAVIGATION_TEMPLATES` permission も無い（第三者が Surface を受け取るのに必須の宣言が存在しない）。`androidx.car.app` の痕跡は connection provider・`CarAppPermissionActivity`・通知 receiver の3つのみで、nav 描画とは無関係。

2. **使用しているのは gms.car / gearhead projection プロトコル。** `com.google.android.apps.gmm.car.GmmCarProjectionService` ほか計5個の service が `com.google.android.gms.car.category.CATEGORY_PROJECTION` / `CATEGORY_PROJECTION_NAVIGATION` を intent-filter に持ち、いずれも meta-data `com.google.android.projection.gearhead.GHOST_ACTIVITY` で `GhostActivity` を指す。`GmmCarProjectionService` の基底クラスは難読化（`bijb`）で追跡不能。

3. **`GhostActivity` は本物の Activity。** decompile の結果 `extends android.support.v4.app.FragmentActivity`（本物の `Window` を持つ）。`onCreate` で `Build.VERSION.SDK_INT >= 33` を要求（"Ghost Activity cannot be started on devices below T"）。`getDisplay().getDisplayId() == 0`（端末メイン画面）の場合は `finishAndRemoveTask()` で即終了 ＝ projection 用セカンダリ Display 専用。log tag は `GH.GhostActivity`（GH = Gearhead）。実 UI は `carActivityServiceComponentName`（car activity service）が駆動。

4. **起動ゲートは gearhead 定義の permission。** `GhostActivity` は `android:allowEmbedded="true"`、`android:exported="true"`、`android:permission="com.google.android.projection.gearhead.permission.START_PROJECTED_ACTIVITY"`。この permission の定義は本 APK に無く gearhead(Android Auto アプリ)側にあるため、protectionLevel は本 APK からは確認不能。

### 事実から確定したこと

- Google Maps は Android Auto UI を「本物の Activity を projection 用セカンダリ Display 上で走らせる」方式で描画しており、第三者用 Car App Library の「生 Surface + semantic callback」モデルは使っていない。
- 本物の Activity ＝ 本物の Window のため、通常の View 描画と host が注入する本物の `MotionEvent` 入力が成立する。第三者が必要とする VirtualDisplay+Presentation・合成 MotionEvent は不要。
- この projection-Activity 経路は署名/gearhead 専用 permission と閉じた `CATEGORY_PROJECTION` intent protocol で第三者から遮断されている。第三者は `androidx.car.app` の生 Surface + semantic callback に限定される。
...
- APK 解析: `com.google.android.apps.maps` `base.apk` の `AndroidManifest.xml`（`GmmCarProjectionService` / `GhostActivity` / `CATEGORY_PROJECTION_NAVIGATION` / `START_PROJECTED_ACTIVITY`）、decompile 済 `GhostActivity`（`extends FragmentActivity`, API33+, secondary display 専用）。成果物はリポジトリ外 `APK/GoogleMap/cc_analysis/`。

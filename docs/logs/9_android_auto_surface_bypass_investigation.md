...
### 6.1 2026-06-06 追記: Google Maps APK デコンパイルで確認できたこと

別作業ディレクトリ `~/dev/APK/GoogleMap/codex_analysis/` で、Pixel 10 から取得済みの
Google Maps APK/APKS を apktool / JADX / native symbol で確認した。

主な成果物:

- `~/dev/APK/GoogleMap/codex_analysis/reports/00_orchestration_log.md`
- `~/dev/APK/GoogleMap/codex_analysis/reports/01_google_maps_android_auto_rendering_findings.md`
- `~/dev/APK/GoogleMap/codex_analysis/subagents/agent_a_manifest_components.md`
- `~/dev/APK/GoogleMap/codex_analysis/subagents/agent_b_dex_surface_rendering.md`
- `~/dev/APK/GoogleMap/codex_analysis/subagents/agent_c_native_strings.md`

確認結果の要点:

1. Google Maps の Android Auto entry point は、公開 AndroidX Car App の
   `CarAppService` / `SurfaceCallback` ではなく、`com.google.android.gms.car.*` と
   `com.google.android.projection.gearhead.*` 側の projection 経路だった。
2. manifest には `GmmCarProjectionService`,
   `LimitedGmmCarProjectionService`,
   `WidescreenWidgetLimitedGmmCarProjectionService`,
   `GmmCarAuxiliaryProjectionService`,
   `GhostActivity` があり、`CATEGORY_PROJECTION_NAVIGATION`,
   `CATEGORY_AUXILIARY_DISPLAY`, `CATEGORY_SECONDARY_REGION`,
   `CATEGORY_CLUSTER_DISPLAY`, `GHOST_ACTIVITY` metadata が使われていた。
3. projection service 群は decompile 上 `defpackage.bijb -> defpackage.bikm` に集約され、
   Binder descriptor は `com.google.android.gms.car.ICarProjection` だった。
4. host から渡る描画仕様は AndroidX `SurfaceContainer` ではなく
   `com.google.android.gms.car.DrawingSpec` で、width / height / dpi / `Surface` /
   window insets / displayId / extras bundle を持つ。
5. Binder transaction では `DrawingSpec` だけでなく、実 `MotionEvent`, `KeyEvent`,
   `InputFocusChangedEvent` も受け取り、`TouchEventCompleteData`,
   `KeyEventCompleteData` で入力完了情報を返していた。
6. 旧経路は `DrawingSpec.Surface -> DisplayManager.createVirtualDisplay(..., flags=10) ->
   Presentation`。SDK の `DisplayManager` 定義で `10` は
   `VIRTUAL_DISPLAY_FLAG_PRESENTATION(2) + VIRTUAL_DISPLAY_FLAG_OWN_CONTENT_ONLY(8)` と確認できた。
7. 新経路は `DrawingSpec.extras["input_transfer_token"] -> SurfaceControlViewHost ->
   SurfacePackage`。`SurfacePackage` は GMS projection callback の transaction code `35` で
   host 側へ返している。これは API 36 / feature flag 条件下で選択される実装に見える。
8. `SurfaceControlViewHost` 自体は public framework API だが、成立には host 側から渡される
   `InputTransferToken` と `SurfacePackage` を受け取る callback contract が必要であり、
   AndroidX Car App の `SurfaceContainer` だけでは再現できない。
9. 地図本体は `libgoogle3.so` 内の native mapcore/vector GL renderer が濃厚。
   `NativeMapFactory`, `NativeMapController`, mapcore `RendererJni`,
   `CameraControllerJni` の JNI entry point、`ANativeWindow_fromSurface`,
   `eglCreateWindowSurface`, `eglSwapBuffers`, GLES draw call、`AChoreographer` import を確認した。
10. `libgmm-jni.so` は `libgoogle3.so` に依存する薄い loader、
    `libsurface_util_jni.so` は AndroidX Camera の `SurfaceUtil` で、地図 renderer 本体ではなかった。

この追跡で、以前の推測は以下のように修正する。

- 「Google Maps first-party は AndroidX Car App の生 `Surface` に自前 renderer を直接つないでいる」
  という単純な構図ではない。
- Google Maps は first-party/GMS projection Binder で、第三者 Car App には渡らない
  `DrawingSpec`, display/region 情報、低レベル入力、場合によっては `InputTransferToken` を受け取る。
- legacy path では Google Maps であっても `VirtualDisplay + Presentation` を使っている。
- API 36+ の新 path では `SurfaceControlViewHost` を使うが、host-provided token と GMS callback が前提で、
  OneNavi が AndroidX `SurfaceCallback` から直接再現できる抜け道ではない。
- Google Maps のリッチさは「projection host が first-party UI 投影を許すこと」と、
  「その UI 内の地図部分が Google Maps 独自 native renderer で動くこと」の組み合わせと見るのが安全。

OneNavi への設計反映:

- Google Maps SDK / Navigation SDK の `View` 系 API を使う限り、引き続き
  `VirtualDisplay + Presentation` を本線とする。
- `SurfaceControlViewHost` は、AndroidX Car App の app-owned raw `Surface` だけでは成立しないため、
  通常実装候補からは外す。
- どうしても Google Maps first-party と同等の host 経路を狙うなら、
  Gearhead / Google Play services car projection 側の permission, allowlist, Binder contract,
  `InputTransferToken` 発行条件を別途解析する必要がある。これは root / system app /
  host patch 前提のハック領域で、通常配布可能な OneNavi 実装とは分けて扱う。
- raw `Surface` 直接描画を追うなら、Google Maps SDK の流用ではなく、
  MapLibre / 独自 EGL renderer / Navigation SDK 以外の地図 renderer 選定として検討する。
